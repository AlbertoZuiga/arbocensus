...
\textit{dataset} de este tamaño. Se reportan ocho indicadores: el número de
rutas $k$ generado de forma automática por el \textit{solver}, el tiempo promedio por
ruta $\bar{T}$, el índice de balance de carga ---cociente entre la duración de la
ruta más corta y la de la más larga, cuyo criterio de aceptación es que alcance al
menos $0{,}80$ (CA-03)---, la desviación estándar de los tiempos de ruta, que
describe la dispersión de las duraciones sin criterio asociado, el número de rutas
que exceden la cota dura $T_{\max}$ ---que debe ser cero por construcción--- y el
número de rutas que quedan por debajo de la cota blanda $T_{\min}$ ---que no tiene
criterio asociado,
por tratarse de un objetivo penalizado y no de una restricción---. A ellos se suman
el número de árboles descartados ---el modelo admite descartes mediante disyunciones
