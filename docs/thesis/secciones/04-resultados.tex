...
(Ecuación~\ref{eq:objetivo}), y conviene tenerlos presentes desde el inicio porque todos los
experimentos del capítulo razonan sobre ellos. El término principal es el costo de arco,
proporcional al tiempo de tránsito entre paradas consecutivas (aproximadamente 1\,s por segundo de
caminata según la red peatonal de OSRM). Sobre él actúan dos penalizaciones blandas en la dimensión
temporal de cada ruta: la penalización inferior, con coeficiente $10\,000\,\text{s}^{-1}$,
desalienta que una ruta quede por debajo de $T_{\min}$; y la penalización superior, con coeficiente
$500\,\text{s}^{-1}$, desalienta que la duración supere el punto medio del intervalo
$[T_{\min},\,T_{\max}]$. Se añaden además un costo fijo de $100\,000$\,s por vehículo activo, que
desincentiva el uso de rutas adicionales, y una penalización de $1\,000\,000$\,s por árbol no
cubierto, que garantiza que la cobertura total sea siempre preferible a cualquier mejora marginal
en los demás términos. El sexto componente es el costo de extensión geográfica, con coeficiente
$\lambda_{\text{span}} = 3$ por metro de \textit{span} \textit{haversine} de cada ruta, que solo
actúa bajo la estrategia \texttt{spatial\_term} y es lo que la distingue de \texttt{global}.
...
sobre una réplica local del entorno de producción definida en \texttt{docker-compose.prod.yml}
---un archivo distinto del que gobierna el despliegue, \texttt{docker-compose.production.yml},
aunque ambos usan las mismas imágenes---. Esta réplica
...
real de OSRM \citep{osrm} cargada con el extracto completo de Chile de \textit{OpenStreetMap}
...
Las corridas de referencia anteriores componían tres de estas celdas desde corridas distintas,
porque las rutas bajo $T_{\min}$, el tiempo extremo a extremo y los auto-cruces sobre calle se
incorporaron al instrumento en ciclos posteriores al de la corrida original; la corrida unificada
de esta sección elimina esa composición. El cambio corrige una de las cifras compuestas: la corrida
de julio de la que provenía el conteo de rutas bajo $T_{\min}$ reportaba tres, y la corrida
unificada no deja ruta alguna bajo el piso en ninguna de las tres repeticiones.
...
rendimiento (\hyperref[anx:perfil-rendimiento]{Anexo 4}, Tabla~\ref{tab:grilla-variantes}) proviene de un
...
criterio de decisión pre-registrado de ese ciclo (\texttt{spatial-span-20260730.md}) y se revierte
en cuanto el coeficiente crece. La lectura obliga además a una
...
rutas, y lo paga en tiempo y en balance; no es una mejora en todos los ejes a la vez. La decisión
pondera esos ejes desde la operación: el sobrecosto de tiempo ($+1{,}5$\,\%) y de balance
($0{,}833$ frente a $0{,}838$) queda dentro de la dispersión entre semillas de la propia
\texttt{global}, mientras que la reducción del solapamiento a menos de la mitad cambia la calidad
territorial del plan ---el eje que la apertura de este apartado ancla en la operación del censo---,
y ese es el intercambio que se adopta. El único eje
...
\texttt{crossings\_road} por área individual, medida con la configuración de producción y tres
semillas (Sección~\ref{sec:areas-reales}), acota lo que ese régimen significa para los auto-cruces:
una solución completa sobre un área real contiene entre 0 y 6 auto-cruces sobre calle (medias de
$1{,}3$ a $4{,}7$ según el área), frente a 24--37 en el conjunto agregado disperso
(Ilustración~\ref{fig:ruta-vueltas-r1}), situación que en operación solo surgiría si se procesaran
múltiples áreas de censo como un único problema. La diferencia es de conteo absoluto y no de tasa:
normalizado por ruta el indicador es esencialmente plano en toda la serie ($1{,}33$--$1{,}56$ en
las áreas y su unión frente a $1{,}25$ en la agregada), de modo que el conjunto agregado no produce
más auto-cruces por ruta, sino más rutas ---$k=25$ frente a $k=1$--$5$--- y, con ellas, el conteo
que la inspección visual percibe.
...
De los resultados anteriores se derivan dos lecturas que conviene dejar explícitas. La primera es
geométrica: en áreas de extensión real ($\leq 1{,}32$\,km) las rutas de la configuración de
producción contienen pocos auto-cruces sobre calle en términos absolutos ---0 a 6 por solución
completa, según la serie medida en la Sección~\ref{sec:areas-reales}, aunque no cero, y con una
tasa por ruta comparable a la del conjunto agregado---, y cuando los auto-cruces aparecen responden
a la función objetivo, no a una limitación del tiempo de cómputo. La segunda es la cercanía del
\textit{baseline} \textit{greedy} en desplazamiento total, que proviene de la saturación: el
heurístico llena cada ruta hasta un $98$\,\% de $T_{\max}$ ---sin dejar holgura operacional---, de
modo que su reducción de caminata proviene de la cota, no de la calidad del ordenamiento. La
diferencia de $4{,}7$\,\% de desplazamiento a favor del \textit{solver} ---medida contra un
\textit{baseline} que recibe el mismo post-proceso de 2-opt--- y la menor severidad de su ruta
residual son las dimensiones que el tiempo total de desplazamiento no captura de forma aislada
(Sección~\ref{sec:comparacion-estrategias}).
...
el arbolado metropolitano agregado: la serie de auto-cruces sobre calle medida en la
Sección~\ref{sec:areas-reales} muestra que una solución sobre un área real contiene entre 0 y 6
auto-cruces (medias de $1{,}3$ a $4{,}7$), frente a 24--37 del conjunto agregado. La medición
corrige, eso sí, la lectura cualitativa que este capítulo sostuvo durante la experimentación: las
rutas de área real no están libres de auto-cruces, y la tasa por ruta es comparable entre regímenes
($1{,}33$--$1{,}56$ frente a $1{,}25$), de modo que la ventaja del área real es de conteo absoluto
---menos rutas y, con ellas, un orden de magnitud menos de auto-cruces por solución--- y no una
geometría por ruta intrínsecamente más limpia. Esto convierte la segmentación previa por área, que
...
$6{,}7$\,\% inicial ---cifra de aquella medición entre corridas no comparables, distinta del
$7{,}5$\,\% que la corrida homogénea mide contra el \textit{greedy} sin refinar (nota 1 de la
Tabla~\ref{tab:comparativa})---: alrededor de un tercio de aquel margen era atribuible al
post-proceso que solo un lado recibía. El margen subsistente supera la dispersión entre semillas
del \textit{solver} ($1\,538$\,s), de modo que el criterio se cumple. Junto a él subsiste una
diferencia cualitativa: el \textit{baseline} produce una ruta degenerada de cinco árboles con
$9\,466$\,s de caminata y satura las jornadas a un promedio del $98$\,\% de $T_{\max}$, mientras
que la solución del modelo opera al $93$\,\% en promedio y su ruta más pequeña visita entre seis y
doce árboles. La atenuación de esa patología es de grado y no de naturaleza: el modelo no la
elimina.
...
censo individual--- la serie medida en la Sección~\ref{sec:areas-reales} acota el indicador: entre
0 y 6 auto-cruces sobre calle por solución completa (medias de $1{,}3$ a $4{,}7$ según el área),
con una tasa por ruta comparable a la de la instancia de estrés, lo que sitúa el cumplimiento del
atributo en el conteo absoluto por solución y no en la ausencia de auto-cruces.
...
ciclo de la serie apareció un brazo que ganara en todas las instancias a la vez. Lo habitual fue lo
contrario: una configuración que mejora la geometría en el régimen agregado empeora el balance en
las áreas pequeñas, una que elimina las rutas cortas paga tiempo de desplazamiento, y el orden entre
