...
pseudoaleatorio que produzca repeticiones distintas (restricción RE-05 de la
Sección~\ref{sec:restricciones}), de modo que cada repetición se construye permutando el orden en
que los nodos entran al \textit{solver}. Ese orden no cambia el problema matemático, pero decide
...
producción; el servidor del despliegue productivo (Sección~\ref{sec:despliegue-produccion}),
incorporado al cierre de la iteración, no es el entorno de medición y aporta únicamente la corrida
de contraste que cierra este apartado. La réplica local emplea las mismas imágenes de contenedor
del despliegue, una instancia real de OSRM \citep{osrm} cargada con el extracto completo de Chile
de \textit{OpenStreetMap} con perfil peatonal, y PostGIS \citep{postgis} como base de datos. Las
...
auto-cruces, por la reserva de instrumento que documenta la Sección~\ref{sec:protocolo-evaluacion}. Se
...
real de calles (\texttt{docs/experiments/areas-crossings-road-20260802.csv}). La Tabla~\ref{tab:areas-crossings}
