...
qué régimen geográfico se sostienen ambas respuestas. La Sección~\ref{sec:resultados-obtenidos} las
responde en ese orden: primero declara el protocolo de evaluación ---instancias, entorno de
medición, presupuesto de cómputo y criterios
(Sección~\ref{sec:protocolo-evaluacion})---, luego acredita la calidad del plan sobre la instancia
de mayor tamaño (Sección~\ref{sec:metricas-calidad}) y su mejora sobre la alternativa trivial
(Sección~\ref{sec:comparacion-greedy}), después mide el efecto del término de dispersión geográfica
(Sección~\ref{sec:comparacion-estrategias}) y el papel del régimen geográfico de la instancia
(Sección~\ref{sec:discusion-calidad}), y cierra con la evaluación sobre las áreas de censo reales
(Sección~\ref{sec:areas-reales}). Las conclusiones (Sección~\ref{sec:conclusiones}) recogen lo que
esos resultados muestran sobre el problema y el cumplimiento de criterios y atributos; las dos
secciones finales registran las limitaciones del sistema y las mejoras posibles.
...
\begin{figure}[H]
  ...
  \caption{
    Nodos de la instancia de referencia ($n = 1\,607$), diferenciados por fuente de origen.
  }
...
Salvo indicación en contrario, los experimentos emplean los parámetros temporales por defecto
($T_{\min} = 7\,200$\,s y $T_{\max} = 10\,800$\,s), un tiempo de servicio de 2 minutos por árbol y
los \textit{defaults} de producción del motor: la estrategia \texttt{spatial\_term} ---que agrega
al objetivo un término de dispersión geográfica, véase la Sección~\ref{sec:estrategias}, y cuya
adopción como \textit{default} se valida en la Sección~\ref{sec:comparacion-estrategias}; aquí se
emplea por ser la configuración de producción vigente--- y un
...
La función objetivo que minimiza el \textit{solver} es la suma ponderada de seis términos de la
Ecuación~\ref{eq:objetivo}, con los coeficientes de producción de la Tabla~\ref{tab:coeficientes};
los experimentos del capítulo razonan sobre esa estructura de precios, en particular sobre las dos
...
Las réplicas de cada corrida no provienen de una semilla de generador pseudoaleatorio
---\textit{OR-Tools} no la expone--- sino de permutar el orden de entrada de los nodos. Esta
permutación altera el desempate en la heurística de construcción inicial
(\texttt{PATH\_CHEAPEST\_ARC}), que recorre los arcos en orden de índice, y propaga la perturbación
a toda la trayectoria del metaheurístico GLS \citep{voudouris1999guided}: aunque el \textit{solver}
es determinista dados sus parámetros de entrada, el orden de los nodos ---información irrelevante
para el problema matemático--- basta para generar soluciones distintas. La sensibilidad al orden de
entrada no se limita al tiempo de desplazamiento: sobre las métricas de calidad alcanza un orden de
magnitud mayor, y las barras de error entre semillas de una corrida no acotan la varianza entre
corridas distintas (\hyperref[anx:alcance-replicas]{Anexo 4}).

\subsubsection*{Entorno de medición}
\phantomsection\label{sec:entorno-medicion}%
Las mediciones de este capítulo preceden al despliegue productivo del sistema
(Sección~\ref{sec:despliegue-produccion}), incorporado al cierre de la iteración, y se realizaron
sobre una réplica local del entorno de producción: las mismas imágenes de contenedor del
despliegue, una instancia real de OSRM \citep{osrm} cargada con el extracto completo de Chile de
\textit{OpenStreetMap} con perfil peatonal, y PostGIS \citep{postgis} como base de datos. Las
...
y 44), cuyos datos crudos ---una fila por semilla--- se versionan en
\texttt{docs/experiments/reference-unified-20260802.csv}; la primera repetición se ejecutó con la
...
\begin{xltabular}{\textwidth}{lXX}
  ...
  \endlastfoot
    ...
    Tiempo de planificación &
      123{,}3\,s de corrida completa con la matriz en caché, de los cuales 120{,}0\,s
...
medir el sistema. La evidencia de que el presupuesto elegido basta es el análisis de sensibilidad
de la Tabla~\ref{tab:sensibilidad-tl}, ya presentado en la
Sección~\ref{sec:protocolo-evaluacion} y que la Sección~\ref{sec:cumplimiento-criterios} retoma.
...
OSRM. El caso frío queda así por sobre el umbral de cinco minutos de CA-05, que la corrida de julio
de 2026 había medido en 299{,}3\,s, al borde; la Sección~\ref{sec:cumplimiento-criterios} recoge
esa lectura.

El comando reproducible y los datos crudos de esta corrida se documentan en el
\hyperref[anx:comandos]{Anexo 3} (\texttt{baseline\_postpass}). Sus cifras y las de la comparación
de estrategias de la Sección~\ref{sec:comparacion-estrategias} provienen de corridas distintas
sobre el mismo arbolado ---con órdenes internos de nodos distintos y, por tanto, trayectorias del
GLS distintas--- y no deben compararse arco a arco.
...
El comando reproducible de los tres brazos, sus datos crudos y el análisis del ciclo ---con el
criterio de comparación fijado antes de ejecutarlos--- se documentan en el
\hyperref[anx:comandos]{Anexo 3} (\texttt{greedy-postpass-20260726}). El heurístico es determinista,
por lo que cada variante se ejecuta una sola vez; el \textit{dataset} es la instancia
\texttt{reference-n1607} congelada en \texttt{docs/experiments/instances/}, cuyos identificadores
son función pura del archivo y por lo tanto reproducibles en cualquier máquina.
...
Las cifras de \texttt{cluster\_first} corresponden a una implementación corregida en dos puntos
respecto de su versión inicial ---la estimación de $k$ emplea el viaje medio al vecino más cercano
en lugar del promedio entre todos los pares, y el presupuesto del \textit{solver} se reparte entre
los grupos en proporción a su tamaño---, sin lo cual la estrategia no era medible en condiciones
homogéneas.
...
entre rutas. No es un veredicto sobre toda instancia ni sobre todo indicador. Dos apartados
posteriores lo acotan: la Sección~\ref{sec:areas-reales} lleva esta misma configuración de
producción a las áreas reales del censo, donde el criterio de decisión se cumple en dos de las tres
y no en las tres; y el barrido del coeficiente espacial que cierra este apartado muestra que la
...
\texttt{cluster\_first} queda descartada con datos, incluso en su versión corregida.
...
\hyperref[anx:perfil-rendimiento]{Anexo 4} proviene de un régimen de medición anterior y no es
...
versionados en \texttt{docs/experiments/} (reportes \texttt{route-quality-20260712.md} y
\texttt{spatial-span-20260730.md}, con los archivos CSV asociados).
...
Sección~\ref{sec:comparacion-greedy}: la configuración de producción de \textit{OR-Tools} con tres
...
régimen de relleno que la Sección~\ref{sec:discusion-calidad} caracteriza para instancias que no
saturan su presupuesto temporal. Con la configuración de producción las cotas
...
La afirmación de la Sección~\ref{sec:discusion-calidad} sobre el régimen de área real se midió en
una corrida propia, posterior al ciclo de partición: configuración de producción, tres semillas por
instancia y auto-cruces contados sobre el trazado real de calles
(\texttt{docs/experiments/areas-crossings-road-20260802.csv}). La Tabla~\ref{tab:areas-crossings}
...
La serie corrige la forma fuerte de la observación cualitativa que este capítulo arrastraba: las
...
anterior del perfil de rendimiento (\hyperref[anx:perfil-rendimiento]{Anexo 4}) y que esa tabla es
la remedición posterior, ya
...
normal, pero el primer cálculo de un \textit{dataset} nuevo lo excede en esta medición ---la
corrida de julio de 2026 había medido $299{,}3$\,s, al borde--- y así se reporta.
