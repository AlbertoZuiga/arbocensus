...
Los árboles son reales; lo sin correlato operativo es tratar esa unión de veintiuna
áreas dispersas como un único trabajo de ruteo (Ilustración~\ref{fig:regimen-disperso-vs-real}).
...
\begin{itemize}
  ...
  matriz de duraciones de OSRM, y la aplicación dibuja al censador la polilínea real de
...
Las mejoras enumeradas a continuación se derivan directamente de los resultados
del capítulo. Las dos primeras tienen anclaje en mediciones propias de esta
memoria; las cuatro restantes abordan limitaciones funcionales independientes de
la serie experimental.

\begin{itemize}
  \item \textbf{Segmentación previa por área}: la
  Sección~\ref{sec:discusion-calidad} y la Sección~\ref{sec:areas-reales}
  establecen que la unidad operativa correcta del motor es el área de censo
  individual. Procesar cada área como problema independiente eliminaría los
  auto-cruces del régimen disperso y reduciría la varianza entre repeticiones.
  El costo está medido sobre las áreas reales: una ruta adicional, tres jornadas
  por debajo de $T_{\min}$ y el triple de cómputo, sin incremento en el
  desplazamiento.
  ...
  penaliza por un factor de $43$ frente a la solución del heurístico. Un
  post-proceso de fusión de paradas entre rutas, operando sobre la solución ya
  construida sin volver a resolver el problema completo, atacaría ese valle sin
  incrementar el presupuesto de cómputo del GLS.

  \item \textbf{Matrices de costos de gran escala}: superar el tope de
  $2\,000$ árboles que hoy acota la dimensión de la matriz de costos ---impuesto
  por el crecimiento $O(n^2)$ de su tamaño en disco--- mediante almacenamiento
  disperso o la segmentación previa del \textit{dataset}, para censos que
  excedan ese umbral.

  \item \textbf{Operación \textit{offline} robusta}: reemplazar la cola
  \textit{offline} básica actual (Sección~\ref{sec:ui}) por un \textit{service
  worker} que gestione el almacenamiento de teselas y la sincronización diferida
  de forma confiable en condiciones de conectividad intermitente.

  \item \textbf{Importación de formatos adicionales}: soporte de GeoJSON como
  formato de entrada de \textit{datasets}, que actualmente se importan desde
  archivos CSV o JSON, o bien desde las fuentes legadas
  (Sección~\ref{sec:legacy}).

  \item \textbf{Pruebas de extremo a extremo}: automatización de los flujos
  completos de administrador y censador mediante Playwright.
\end{itemize}

Una séptima extensión se evaluó y quedó descartada por la propia medición, lo
...
De todas las extensiones enumeradas, la de mayor valor para el proyecto es la
validación en terreno de la plataforma en una jornada real de censo. Las
conclusiones de este capítulo verifican que el producto hace lo que su diseño
declara, pero no que resuelva en operación el problema logístico que lo motivó.
Esa comprobación queda pendiente y ninguna mejora de la función objetivo la
sustituye.
