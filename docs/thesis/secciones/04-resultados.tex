...
\begin{enumerate}[label=\alph*)]
  ...
  \item el tiempo de cómputo del \textit{solver}.
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lXX}
    ...
    Tiempo de cómputo (\textit{solver}) & 120{,}0\,s = tope configurado, agotado en las tres repeticiones & $< 5$ min (CA-05); se cumple por construcción ---la evidencia de suficiencia es la Tabla~\ref{tab:sensibilidad-tl} \\
    \bottomrule
  \end{tabularx}
\end{table}
...
(Anexo~\ref{anx:grilla-variantes}).
...
defectos se documenta en el Anexo~\ref{anx:auditoria-robustez}.
...
considerarse al interpretar las cifras absolutas. El hardware es un Apple
...
Anexo~\ref{anx:grilla-variantes} (\autoref{tab:grilla-variantes}).
...
lo motivaba. De los dieciséis ciclos, doce cerraron con un descarte, dos fueron
correcciones del propio instrumento de medición, uno caracterizó la flojedad de la
cota inferior de referencia y uno adoptó un cambio. Los doce descartes fueron:
\begin{enumerate}[label=\alph*)]
...
\end{enumerate} La conclusión que emerge del conjunto no es que no se encontró
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{Reconciliación de trabajos abandonados}: el registro de un
    trabajo de optimización permanece en estado \texttt{running} si el
    \textit{worker} muere después de iniciar la tarea; el sistema no implementa
    reconciliación por latido ni por marca de inicio, y el cliente abandona el
    sondeo a los 15 minutos (Sección~\ref{sec:ui}). La consecuencia es un registro
    colgado en la lista de trabajos, sin efecto sobre las soluciones ya publicadas.
  ...
  \begin{itemize}
    ...
    para todo el \textit{dataset} (Anexo~\ref{anx:efectos-no-medidos}).
    ...
    (Anexo~\ref{anx:efectos-no-medidos}).
  \end{itemize}
  ...
  \begin{itemize}
    ...
    Este solapamiento persiste con la estrategia \texttt{spatial\_term} porque responde
    a la geometría local del par, y ningún coeficiente global lo elimina sin degradar
    las rutas circundantes; la solución estructural es la segmentación previa por área
    de censo.
