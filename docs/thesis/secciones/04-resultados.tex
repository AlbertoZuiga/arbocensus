...
Tabla~\ref{tab:sensibilidad-tl}, medido en el régimen anterior de la
Sección~\ref{sec:perfil-rendimiento} ---estrategia \texttt{global} y código previo a
la corrección del redondeo del tránsito, de donde proviene también su $k=24$---, y
que la Sección~\ref{sec:cumplimiento-criterios} retoma.
...
El resultado adverso admite un diagnóstico preciso: no dice lo que a primera vista parece decir. Con la configuración de producción
...
altera la solución, y la re-medición posterior sobre \texttt{crossings\_road}, que
...
tope se bajó a 120\,s a partir del análisis de sensibilidad de la
Tabla~\ref{tab:sensibilidad-tl}. El compromiso entre balance, flota y costo
...
\begin{table}[H]
  \centering
  \caption{Sensibilidad al límite de tiempo del \textit{solver} (servicio de 2\,min, $T_{\max}=3$\,h, $n=1\,607$; estrategia \texttt{global}, código previo a la corrección del redondeo del tránsito, de ahí que $k=24$ y no $25$ como en el resto del capítulo).}
...
El $k=24$ de esta tabla proviene de una medición anterior a la corrección del
redondeo del tránsito descrita en la Sección~\ref{sec:discusion-calidad}; no
contradice el $k=25$ del resto del capítulo, que corresponde al código vigente.
...
captura prácticamente toda la mejora alcanzable con presupuestos mayores; conviene
registrar que esa tabla se midió en el régimen anterior de la
Sección~\ref{sec:perfil-rendimiento}, con la estrategia \texttt{global} y el código
previo a la corrección del redondeo del tránsito, y que de ahí proviene su $k=24$.
...
presupuesto captura prácticamente toda la mejora alcanzable: ampliarlo a $180$\,s
altera el desplazamiento total en un $0{,}1$\,\% y no cambia ni el número de rutas ni
el balance. La salvedad pertinente es que la cifra de $120$\,s es el tope configurado
...
\begin{itemize}[label=--]
  ...
  \item \textbf{Post-proceso de fusión de rutas}: el diagnóstico de
  \texttt{area-27} en la Sección~\ref{sec:areas-reales} muestra que el
  \textit{solver} queda atrapado en un óptimo local cuya función objetivo lo
  penaliza por un factor de $43$ frente a la solución del heurístico. El costo es
  de implementación, no de cómputo adicional: opera sobre la solución ya
  construida. Debe construirse un post-proceso de fusión de paradas entre rutas
  que ataque ese valle sin incrementar el presupuesto del GLS.
