...
\hyperref[anx:suite-instancias]{Anexo 4}. Este apartado reporta en detalle una de ellas, la
instancia de referencia descrita en la Sección~\ref{sec:legacy} y en la
Sección~\ref{sec:estado-actual} (ambas fuentes, \texttt{legacy\_api} + \texttt{legacy\_app} con
mediana de posición por código QR), de $N = 1\,607$ árboles, porque es la que fija los criterios de
aceptación del Capítulo~\ref{cap:solucion}. Las once restantes ---entre ellas tres áreas de censo
individuales de la operación histórica--- sostienen los apartados siguientes, y conviene anticipar
que no siempre coinciden con ella: la Sección~\ref{sec:areas-reales} muestra que varias conclusiones
obtenidas en el régimen agregado no se trasladan sin más al régimen de área individual, que es el de
la operación habitual. El conjunto no corresponde
...
\begin{enumerate}[label=\alph*)]
  ...
  \item el número de auto-cruces sobre el trazado real de calles, descriptivo y sin criterio
     en esta corrida.
\end{enumerate}
La Tabla~\ref{tab:metricas} resume estos resultados ---los de la configuración de producción recién
descrita: estrategia \texttt{spatial\_term} y presupuesto de \textit{solver} de 120\,s--- como el
promedio de tres repeticiones (semillas 42, 43 y 44), y remite, en su columna de criterio, a los
criterios de aceptación comprometidos en la Sección~\ref{sec:criterios-aceptacion}.

\begin{xltabular}{\textwidth}{lXX}
  \caption{Métricas de calidad de rutas sobre la instancia de referencia.}
  \label{tab:metricas} \\
  \toprule
...
Sección~\ref{sec:cumplimiento-criterios} retoma. Su lectura en simple es que conceder más tiempo ya
no compra nada: el número de rutas no se mueve, y pasar de 120\,s a 180\,s mejora el desplazamiento
un $0{,}6$\,\%, frente al $3{,}5$\,\% que se gana al pasar de 60\,s a 120\,s.
...
Tras el despliegue se ejecutó una medición de contraste sobre el servidor de producción (1--2 de
agosto de 2026), con el mismo comando \texttt{baseline\_sweep} del
\hyperref[anx:comandos]{Anexo 3} sobre tres instancias congeladas (\texttt{area-29-n43},
\texttt{area-26-n157} y \texttt{reference-n1607}), la estrategia \texttt{global}, tres
repeticiones y el límite de \textit{solver} vigente de 120\,s (archivos
\texttt{20260802-droplet-profile-*.csv} en \texttt{docs\slash experiments\slash}). El contraste
confirma ambos regímenes en el hardware real. Con la caché caliente, el \textit{pipeline}
completo tomó entre 120{,}0 y 122{,}7\,s según la instancia: las fases fuera de \texttt{solve}
suman menos de 3\,s incluso con $n=1\,607$, de modo que el carácter \textit{solve-bound} se
mantiene en una máquina de un solo núcleo. En frío, la consulta a OSRM vuelve a dominar y es
donde el hardware inferior se paga: las 100 solicitudes de la matriz de $n=1\,607$ costaron
780{,}9\,s frente a los 170{,}8\,s de la réplica local (un factor de 4{,}6), con lo que el
\textit{pipeline} frío completo ronda los 15\,min, frente a los cerca de 5\,min que suman las
mismas fases en la réplica local con igual presupuesto de \textit{solver}. Este costo se sigue pagando
una sola vez por \textit{dataset}, por lo que la operación normal del sistema desplegado queda
acotada por el presupuesto del \textit{solver}, igual que en la réplica local.
