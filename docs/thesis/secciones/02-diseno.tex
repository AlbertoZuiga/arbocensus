...
solver. El administrador nunca declara cuántos equipos tiene disponibles; declara
...
En su lugar, el sistema entrega a OR-Tools un \textbf{techo} $N_{\max}$ de vehículos
disponibles y deja que el solver decida cuántos activa. El techo se calcula a partir
...
$\hat{c}_{\text{nn}}$ el \textbf{tiempo medio al vecino más cercano}: el promedio,
...
Es importante subrayar que $N_{\max}$ es una \textbf{cota superior}, no una
predicción: solo debe ser lo bastante amplia como para no excluir la solución óptima.
La minimización efectiva de $k$ ocurre dentro del solver, mediante el costo fijo de
$100\,000$\,s por vehículo activo (Sección~\ref{sec:solver}): activar una ruta
adicional es tan caro que el solver solo lo hace cuando las restricciones temporales
...
y no al solver: el \textit{endpoint} \texttt{GET /api/optimization/estimate/} permite
...
\begin{itemize}
  ...
  \item \textbf{Disjunciones por nodo}:
        cada árbol puede quedar fuera de la solución a un costo de
        $1\,000\,000$ (Sección~\ref{sec:disjunciones}).
...
Si el solver no encuentra solución, el \textit{pipeline} no falla en silencio: emite
...
% ---- 2.3.5 ------------------------------------------------------
\subsection{Disjunciones de nodos}
\label{sec:disjunciones}

Un modelo VRP en el que todos los nodos son obligatorios es un modelo que puede
resultar \textbf{infactible}. Basta un único árbol cuyo tiempo de acceso ---ida y
vuelta desde cualquier otro nodo, más su tiempo de servicio--- exceda la cota dura
$T_{\max}$ para que no exista ninguna asignación válida y el solver retorne sin
...
(Sección~2.3.2). Sin un mecanismo de escape, un solo árbol de este tipo invalidaría
...
\textit{disjunción} unitaria mediante \texttt{AddDisjunction}, asignándole una
...
al menos una solución ---aquella que descarta los nodos problemáticos--- y el solver
...
el comportamiento del solver: descartar un árbol es diez veces más caro que abrir una
ruta entera para él. En consecuencia, mientras exista alguna ruta factible que pueda
absorber al árbol, el solver siempre preferirá crearla antes que omitirlo, y el
descarte queda reservado a los nodos que ninguna ruta puede alojar sin violar la cota
dura $T_{\max}$. La disjunción actúa entonces como válvula de seguridad, no como
...
(Capítulo~\ref{cap:resultados}), el efecto no se manifiesta como un mayor tiempo total
sino como una menor calidad alcanzable dentro del mismo presupuesto.
[DATO PENDIENTE: no se ha medido una ablación con y sin disjunciones sobre el mismo
\textit{dataset} y presupuesto de solver; cuantificar esa diferencia de calidad
requiere una corrida controlada adicional.]
...
anteriores del proyecto Arbocensus, que contienen el arbolado ya catastrado en
campañas previas. Ambas se incorporan como fuentes de importación, identificadas por
...
\begin{itemize}
  ...
  \item \texttt{legacy\_app} (esquema \texttt{arbocensus\_app\_*}): base de la
        aplicación móvil previa, donde el árbol no tiene una posición canónica sino
        ...
        \textbf{mediana} de las latitudes y longitudes de sus muestras, agrupadas por
        código QR, descartando las lecturas nulas o de valor cero. Se emplea la
...
$n = 1\,607$ árboles. El acceso a las bases legadas es estrictamente de \textbf{solo
lectura} (la conexión se abre con \texttt{readonly}), dado que se trata de sistemas
que no pertenecen al alcance de este trabajo y cuya integridad no debe verse
...
\texttt{LegacySelectionMap}) presenta sobre un mapa Leaflet la totalidad del arbolado
...
\texttt{TreeObservation} con el campo \texttt{source} marcando su procedencia. El
...
las imágenes residen en el almacenamiento del sistema anterior; solo se importan
aquellas cuya referencia es una URL HTTP resoluble, descartando las rutas locales del
dispositivo que la aplicación móvil previa registraba y que hoy no apuntan a nada.
...
\begin{itemize}
  ...
  \item \textbf{Datasets} (\texttt{/api/datasets/}): importación de archivos CSV con detección automática de columnas de coordenadas, exploración de las fuentes legadas y creación de \textit{datasets} a partir de una selección (Sección~\ref{sec:legacy}), consulta de estadísticas y exportación de árboles como \textit{GeoJSON FeatureCollection}.
  \item \textbf{Optimización} (\texttt{/api/optimization/}): estimación de flota previa a la optimización, creación de trabajos, consulta de estado (para \textit{polling} desde el frontend), acceso a las métricas de cada \texttt{RoutingSolution} y publicación o despublicación de la solución vigente por \textit{dataset}.
...
servicio) y lanza el trabajo de optimización. En
