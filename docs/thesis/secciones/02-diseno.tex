...
% Las capturas de pantalla se depositan en docs/thesis/media/. Mientras un archivo
% no exista, la figura se compila como marco vacío con la ruta esperada.
\newcommand{\capturaui}[3]{%
  \begin{figure}[H]
    \centering
    \IfFileExists{media/#1}%
      {\includegraphics[width=0.9\textwidth]{media/#1}}%
      {\fbox{\parbox[c][5cm][c]{0.86\textwidth}{\centering\ttfamily media/#1}}}
    \caption{#2}
    \label{#3}
  \end{figure}%
}
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Arquitectura de Arbocensus. Trazo continuo: capas del monolito Django. Trazo discontinuo: servicios de apoyo desplegados como contenedores independientes.}
...
La Figura~\ref{fig:er} presenta el modelo entidad-relación simplificado. Se omiten los
atributos no estructurales y la relación auxiliar entre \texttt{TreeObservation} y el
usuario que la registra (\texttt{created\_by}, anulable por la misma razón que
\texttt{surveyor}). Todas las entidades usan claves primarias UUID.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption{Modelo entidad-relación simplificado. \texttt{DistanceMatrix} cachea la matriz de tiempos del \textit{dataset}; \texttt{RouteStop} es la fuente de verdad del orden de visita; \texttt{TreeObservation} sobrevive al plan de rutas que la originó.}
  \label{fig:er}
\end{figure}
...
presupuesto de solver; es el modo con que se produjeron las comparaciones del
Capítulo~\ref{cap:resultados}. La estrategia \texttt{global} reproduce el
comportamiento de la formulación anterior sin cambios y es la seleccionada por
defecto.
...
\capturaui{ui-legacy-import.png}{Mapa de selección de arbolado legado (\texttt{/admin/datasets/legacy-import}): polígonos de las áreas de censo, marcadores coloreados por estado (disponible, seleccionado, ya importado) y panel lateral de la selección acumulada.}{fig:ui_legacy}
...
\capturaui{ui-datasets.png}{Listado de \textit{datasets} e importación de archivos (\texttt{/admin/datasets}).}{fig:ui_datasets}

\capturaui{ui-optimization.png}{Detalle de un \textit{dataset} (\texttt{/admin/datasets/<id>}): mapa de árboles, formulario de parámetros temporales y de estrategia, y panel de seguimiento del trabajo de optimización.}{fig:ui_optimization}

\capturaui{ui-routes.png}{Rutas de una solución (\texttt{/admin/datasets/<id>/jobs/<job>}): pestañas de estrategia con sus métricas, polilíneas coloreadas sobre la red peatonal y asignación de censadores.}{fig:ui_routes}
...
\capturaui{ui-surveyor.png}{Portal del censador (\texttt{/}, vista \textit{mobile}): parada activa, distancia a la parada, captura fotográfica y registro de visita u omisión.}{fig:ui_surveyor}
