...
Tabla~\ref{tab:sensibilidad-tl}), las mismas tres semillas (42, 43 y 44) y la
...
criterio protege: deja \textbf{12 árboles fuera} de la solución de forma
consistente en las tres repeticiones, emplea $k=34$ rutas ($+36$\,\% de flota),
degrada el balance a $0{,}698$ y encarece el desplazamiento total en un
...
$\approx 0{,}83$. El número de rutas coincide variante a variante con el que
produce \texttt{global}, lo que confirma que el término espacial \textit{no}
infla la flota. La observación operacional relevante es que las variantes de
...
No se ejecutó un ajuste fino del coeficiente \texttt{SPAN\_COEF}. La razón es
que \texttt{spatial\_term} ya satisface el criterio de decisión con su valor de
fábrica y consume solo $1{,}5$ de los 10 puntos porcentuales de desplazamiento
adicional admitidos, mientras que el único eje con señal pendiente ---el IoU del
peor par--- es, como se argumentó, un problema de geometría local que un
...
\texttt{SPAN\_COEF} $=3$, un límite de solver de 120\,s y la configuración
censal de referencia de 2\,min de servicio con $T_{\max}=3$\,h;
