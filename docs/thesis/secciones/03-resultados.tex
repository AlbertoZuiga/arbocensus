...
reportado (58\,351\,s, con post-pass 2-opt habilitado por defecto; 59\,898\,s
sin post-pass en la corrida original) y el de la corrida de comparación de
estrategias para la misma variante (60\,611\,s) difieran; ambas cifras
provienen de corridas distintas y no deben compararse arco a arco.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lXX}
    ...
    Tiempo total de desplazamiento & 58\,351\,s$^*$ ($\approx$ 16\,h 12\,min) & 62\,585\,s ($\approx$ 17\,h 23\,min) \\
...
{\footnotesize$^*$ Cifra con post-pass 2-opt habilitado por defecto desde 2026-07-24.
Valor original sin post-pass: 59\,898\,s. El ahorro contra \textit{greedy} es ahora
4\,234\,s ($-6.3$\,\%), cerca de 70 minutos de trabajo de terreno.}

En la métrica que el modelo efectivamente minimiza ---el tiempo total de
desplazamiento--- \textit{OR-Tools} supera al \textit{baseline}. Las otras dos filas de la tabla favorecen aparentemente al
...
La serie experimental descrita registra dieciséis ciclos de evaluación cerrados entre
julio y agosto de 2026. En el análisis retrospectivo de la evidencia, se identificaron
cuatro defectos de instrumentación en ciclos anteriores. Primero, el manejo de semillas
en las corridas iniciales (\#194 a \#217) no propagaba la semilla al \textit{solver};
las nominales tres réplicas por celda eran idénticas salvo por el corte por reloj de
pared, de modo que la dispersión reportada ($\sigma \approx 0{,}03\,\%$) es artefacto de
replicación idéntica, no convergencia del algoritmo. Una corrida de recalibración (\#221)
propagó semillas reales al solver, produciendo $\sigma \approx 2{,}4\,\%$ y modificando
catorce de veintiocho veredictos de ese ciclo. Segundo, la métrica de relleno en esos
mismos ciclos usaba como cota inferior el producto $n \times \bar{n}$, que viola la
...
Una validación posterior (\#240 y \#241) midió correlación entre cuerdas y trazado real de
calles: $\rho$ Spearman global = 0{,}527 (proxy parcial), e invertida en el \textit{dataset}
de evaluación principal ($\rho = -0{,}575$). La prueba decisiva es que el aumento reportado
de auto-cruces tras aplicar 2-opt (reestructuración de secuencia intra-ruta) decía
multiplicarse por 2{,}9 a 5{,}3 veces sobre cuerdas; sobre la geometría real de calles
ese mismo algoritmo reduce cruces en 104 de 108 instancias de prueba, nunca aumenta.
La métrica de cuerdas validó, no obstante, su valor comparativo: todos los ciclos se
contrastaron con la misma vara, de modo que las diferencias relativas entre ellas
...
la duración (menos de 1\,800 segundos). Una ruta con tres paradas y 10\,485\,s satisface
cualquier operación como un turno de 2,9\,h, aunque viole el conteo. El defecto afectó
el descarte de candidatos en ciclos posteriores; una revisión de criterio descartó la
cláusula de paradas como proxy defectuoso, absolviendo cuarenta y tres de ciento cuatro
filas que solo violaban el conteo.

La serie se audita a sí misma: seis ciclos posteriores adoptaron correcciones de
metodología (re-corridas con semillas reales, métrica de relleno MSF, validación de
cuerdas contra calles, re-calibración de criterio de degeneración). Ninguno de esos
ciclos es un fracaso; todos ellos son el descubrimiento y corrección de defectos
propios. El impacto máximo ocurrió donde una corrección de instrumento cambió un
veredicto: el algoritmo 2-opt fue rechazado (\#196 y \#209) porque empeoraba auto-cruces
medidos en cuerdas; la validación de cuerdas (\#240 y \#241) reveló que la métrica era
artefacto de dibujo y que el 2-opt de hecho limpiaba la geometría real. Esa captura
---corrección de instrumento, cambio de veredicto, adopción de default--- ocurrió en
el ciclo M23 (\#245), donde el post-pass 2-opt fue asumido como configuración de
producción con un argumento metodológico puro: quince ciclos barrieron términos del
objetivo sin mover ningún default; lo que cambió fue arreglar el instrumento que medía
la calidad. Ese es el resultado más citable de la serie: no es una ganadora
configuracional, sino una lección de método. La documentación íntegra del análisis
de robustez está disponible en \texttt{docs/experiments/evidence-audit-20260725.md}.
