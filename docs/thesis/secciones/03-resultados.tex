...
operación distinto: el heurístico satura cada ruta hasta el $98{,}5$\,\% de
$T_{\max}$, mientras que las penalizaciones blandas del modelo empujan las
duraciones hacia el punto medio del intervalo y dejan holgura operacional. La
...
proviene de la cota, no de la calidad del ordenamiento. La diferencia de $6{,}7$\,\%
...
trabajo futuro en la Sección~\ref{sec:posibles-mejoras}. El problema que esa extensión
...
La mejora sobre la alternativa trivial (CA-06) se cumple con la reserva más seria de
este capítulo. El ahorro medido es de $4\,221$\,s, un $6{,}7$\,\% del desplazamiento
total del \textit{baseline}, pero las dos cifras que se restan provienen de corridas
distintas sobre reconstrucciones del mismo conjunto de árboles, y el heurístico es
sensible al orden interno de los nodos, que difiere entre ellas. A eso se añade una
asimetría de tratamiento: la solución del \textit{solver} recibe el post-proceso de
resecuenciación de la Sección~\ref{sec:postpass} y el \textit{baseline} no recibe
refinamiento local alguno. La magnitud exacta del margen queda, por tanto, sin
establecer, y cerrarla exige re-correr el heurístico sobre la misma instancia y
dotarlo del mismo post-proceso. Lo que no depende de ese margen, y sí está
establecido, es la diferencia cualitativa: el \textit{baseline} produce una ruta
degenerada de cuatro árboles con $9\,267$\,s de caminata y satura las jornadas al
$98{,}5$\,\% de $T_{\max}$, mientras que la solución del modelo no presenta ninguna de
las dos patologías.
...
\begin{itemize}
  ...
  \textit{OR-Tools} frente al heurístico del vecino más cercano es de solo un
  $6{,}7$\,\% del desplazamiento total (Tabla~\ref{tab:comparativa}). Esta proximidad es un artefacto
  ...
  que se detiene a censar. La magnitud de esa desviación \textbf{no está medida} en
