...
docker compose -f docker-compose.prod.yml run --rm backend \
  python manage.py baseline_sweep \
    --dataset 222fd8ff-44a4-43ed-8f3e-8f5b61ff6110 --strategies spatial_term \
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{lXXXXXXX}
    \toprule
    Estrategia & $k$ & Balance & Descart. & Desplaz. [s] & $\sum r_{\max}$ [m] & Solap./ruta & IoU peor par \\
    \midrule
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{rrXXXXXXX}
    \toprule
    Serv. [min] & $T_{\max}$ [h] & $k$ & Balance & $\sigma(T)$ [s] & Descart. & Desplaz. [s] & $\sum r_{\max}$ [m] & Solap./ruta \\
    \midrule
...
% ---- 3.1.4 ------------------------------------------------------
% Espacio reservado para la subsección «Discusión de calidad de rutas», que se
% insertará aquí como \subsection al cierre de la Sección 3.1. Al ser la última
% subsección de esa sección, su inserción no altera la numeración de las
% secciones siguientes (3.2 y 3.3), que son \section propias.
...
final de la Sección~\ref{sec:variantes-duracion}). Ese régimen era el vigente
durante el perfilamiento, y son precisamente sus resultados ---el análisis de
sensibilidad de la Tabla~\ref{tab:sensibilidad-tl}--- los que motivaron el
cambio de los \textit{defaults} de producción a \texttt{spatial\_term} con un
límite de 120\,s; el perfil documenta, por tanto, el estado previo a esa
decisión y no se reejecutó bajo el régimen nuevo. En consecuencia, sus tablas
---en particular la grilla de seis variantes de la
Tabla~\ref{tab:grilla-variantes}--- \textbf{no son comparables arco a arco} con
las de la Sección~\ref{sec:comparacion-estrategias}, y no deben cruzarse con
ellas como si midieran lo mismo: cualquier diferencia entre ambas estaría
confundida entre tres causas simultáneas (estrategia, redondeo del tránsito y
presupuesto de solver). La sección se conserva porque sus hallazgos son de
\textit{tiempo} ---la dominancia del GLS, el carácter \textit{OSRM-bound} en
frío y la sensibilidad de la calidad al presupuesto del solver--- y ninguno de
los tres depende de esos factores.
