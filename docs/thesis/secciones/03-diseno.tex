...
La Ilustración~\ref{fig:ui_legacy} muestra el mapa de selección con una sesión en curso.
...
\begin{enumerate}
  ...
  \item \textbf{Presentación, recomendación y publicación.} Las rutas resultantes se presentan como polilíneas coloreadas sobre la red peatonal real; \texttt{StrategyTabs} permite contrastar las soluciones de cada estrategia. La función \texttt{pick\_recommended} señala automáticamente la solución preferible según criterio lexicográfico; \texttt{suggest\_assignment} propone la asignación de rutas a censadores que minimiza el déficit de carga acumulado entre campañas. El administrador publica la solución elegida y asigna las rutas.
\end{enumerate}

La Ilustración~\ref{fig:ui_datasets} presenta el listado de \textit{datasets} disponibles en el sistema.
...
La tabla muestra el nombre de cada \textit{dataset}, el número de árboles y la fecha de importación; los botones superiores dan acceso a las dos rutas de creación: subida de archivo y selección desde las fuentes legadas.

La Ilustración~\ref{fig:ui_optimization} muestra la pestaña de optimización del \textit{dataset}.
...
La Ilustración~\ref{fig:ui_routes} muestra la pestaña de asignación con las tres estrategias comparables.
...
La Ilustración~\ref{fig:ui_progress} muestra el tablero de avance de un \textit{dataset} en curso.
...
La barra de progreso global y el desglose por ruta permiten identificar qué rutas se han detenido; el mapa colorea cada árbol según su estado ---censado, omitido o pendiente--- para ubicar geográficamente el avance del censo.
