...
Django \citep{django} con módulos separados por dominio
...
La Ilustración~\ref{fig:secuencia} detalla ese recorrido en el tiempo, para uno de los
seis trabajos que despliega una solicitud (Sección~\ref{sec:abanico}). La petición HTTP
termina en cuanto los trabajos quedan encolados, de modo que el navegador nunca espera
al \textit{solver}; el estado se obtiene después por \textit{polling}. Un fallo del
\textit{worker} deja el trabajo en \texttt{failed} con el mensaje de la excepción, que
el panel muestra sin necesidad de un canal de error distinto del de estado.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \draw[msj] (3.1,-2.1) -- (15.2,-2.1) node[et] {crea la \texttt{RoutingConfig} y seis \texttt{OptimizationJob} en estado \texttt{queued}};
    ...
    \draw[msj] (9.3,-9.7) -- (15.2,-9.7) node[et, align=center]
      {persiste \texttt{RoutingSolution}, \texttt{Route} y \texttt{RouteStop}\\ en una transacción; estado \texttt{completed}};
  ...
  \end{tikzpicture}
  \caption[Ciclo de vida de un \texttt{OptimizationJob}]{Ciclo de vida de un
  \texttt{OptimizationJob}, desde la solicitud del administrador hasta la solución
  persistida. La petición HTTP retorna al encolar; el \textit{worker} avanza el estado
  del trabajo, y el panel lo observa por \textit{polling} hasta un estado terminal
  (\texttt{completed} o \texttt{failed}).}
  \label{fig:secuencia}
\end{figure}

Considerando la naturaleza del problema de optimización, cuyo desarrollo requiere
una cantidad considerable de pruebas y experimentos para evaluar y ajustar la
solución, las decisiones de \textit{stack} se tomaron antes de la construcción del
sistema y se guiaron por tres criterios, formulados de acuerdo con las necesidades
previstas para la iteración y el proceso de desarrollo.

\begin{enumerate}
  \item \textbf{Costo.} Se anticipó un volumen elevado de ejecuciones durante el
        desarrollo y la evaluación experimental del sistema. Bajo ese escenario, un
        componente tarificado por consulta convierte cualquier precio unitario en un
        gasto difícil de acotar. El criterio fue mantener en cero el gasto de la
        etapa de desarrollo. Este criterio califica las alternativas para esa etapa;
        no excluye adoptar más adelante un componente de pago cuyo aporte al
        producto en operación justifique su costo recurrente.
  \item \textbf{Tiempo de ejecución.} Se descartan las latencias que no provienen
        del trabajo útil: la ida y vuelta a un servicio remoto por cada consulta de
        ruteo, o una implementación que recalcule lo ya calculado. El presupuesto de
        tiempo del sistema se reserva para el trabajo propio del problema.
  \item \textbf{Simplicidad del desarrollo.} Entre soluciones de resultado
        equivalente se prefiere la que deja menos piezas que operar y mantener, dado
        que en la iteración actual del proyecto se cuenta con un solo desarrollador.
        La simplicidad cede cuando la alternativa compleja aporta una capacidad que
        el producto necesita.
\end{enumerate}

Los tres criterios se aplican de forma conjunta, sin un orden de prioridad fijo. La
Tabla~\ref{tab:stack-decisions} registra las alternativas evaluadas para cada
componente y el criterio que resolvió la elección.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}l X X@{}}
    ...
    \midrule
    Django + GeoDjango & FastAPI (empleado por la iteración anterior), Flask, \textit{Ruby on Rails} & Simplicidad. GeoDjango es la única de las cuatro que integra PostGIS en el ORM: tipos geográficos, consultas espaciales y migraciones vienen resueltos. FastAPI y Flask exigirían construir esa capa sobre \textit{GeoAlchemy}, y \textit{Rails} ---con extensión espacial equivalente vía \textit{RGeo}--- introduciría un segundo lenguaje en un proyecto cuyo \textit{solver} y cuya ciencia de datos son de Python. \\[4pt]
    \textit{OR-Tools} & \textit{Gurobi}, \textit{CPLEX}, heurística propia & Costo y simplicidad. \textit{Gurobi} y \textit{CPLEX} resuelven mejor el VRP exacto, pero su licencia comercial es un gasto recurrente y su licencia académica no acompaña al producto fuera de la titulación. \textit{OR-Tools} es libre, trae metaheurísticas de VRP ya construidas y se invoca desde el mismo proceso Python; la heurística propia se descartó por costo de construcción frente a un aporte incierto. \\[4pt]
    OSRM & \textit{Google Maps Platform} (\textit{Distance Matrix} y \textit{Routes}), \textit{Mapbox Directions}, Valhalla, GraphHopper & Costo y tiempo de ejecución. Las dos primeras cobran por consulta, y el tamaño esperado del problema hacía previsible el orden de magnitud: una matriz de $1\,607$ árboles supera el millón de pares origen-destino, y esa cifra se paga entera cada vez que se reconstruye. A ello se suma que cada bloque de consultas paga latencia de red y cuotas de tasa, que fijan un piso al tiempo de construcción de la matriz. El beneficio marginal sobre OSRM es acotado, porque el perfil peatonal se resuelve sobre la misma red de \textit{OpenStreetMap}. OSRM se preprocesa localmente, responde sin salir del \textit{host} y deja la red congelada en un archivo, condición sin la cual los experimentos del Capítulo~\ref{cap:resultados} no serían repetibles meses después sobre la misma base. \\[4pt]
    \textit{React} 18 + Vite & \textit{Vue}, \textit{Angular}, \textit{Svelte} & Simplicidad. Las cuatro cubren los requisitos de interfaz; \textit{React} se eligió por ser el ecosistema con integración más directa con \textit{Leaflet} (\textit{react-leaflet}) y con la biblioteca de estado de servidor adoptada (\textit{React Query}), y por ser el que el desarrollador ya dominaba. \\[4pt]
    \textit{Leaflet} & \textit{MapLibre GL}, \textit{Google Maps JavaScript API} & Costo y simplicidad. La API de \textit{Google} requiere clave facturada por carga de mapa. \textit{MapLibre GL} exige un servidor de teselas vectoriales propio, una pieza más de infraestructura que el alcance de visualización no justifica. \textit{Leaflet} consume teselas de \textit{OpenStreetMap} sin costo. \\[4pt]
    Celery + Redis & Ejecución síncrona, \textit{cron} sobre tabla de tareas & Tiempo de ejecución y simplicidad. La construcción de la matriz de costos y la ejecución del \textit{solver} tardan varios minutos: resolverlas dentro de la petición HTTP excede los tiempos de respuesta aceptables de un servidor web, y un sondeo periódico sobre una tabla de tareas añade al trabajo la espera hasta el siguiente ciclo. Celery ejecuta el trabajo apenas se encola y libera la petición de inmediato, a cambio de dos procesos adicionales ---\textit{worker} y \textit{broker}--- que se declaran en la misma composición de contenedores que el resto del sistema. \\[4pt]
    Monolito modular & Servicios separados por dominio & Simplicidad. Las operaciones de importación y optimización cruzan módulos en una sola transacción; el proyecto cuenta con un único desarrollador, y la escala prevista no justifica la complejidad operacional de desplegar servicios independientes. \\
    \bottomrule
  \end{tabularx}
\end{table}

El patrón que recorre la tabla es que ninguna decisión se resolvió por rendimiento
aislado del componente: donde existía una alternativa de pago, su aporte no
compensaba el gasto proyectado sobre el volumen de ejecuciones que el desarrollo
haría necesario ni la latencia que introduce, y entre las alternativas libres
decidió la simplicidad de operarlas. Ninguna de estas elecciones es definitiva: en
operación regular el sistema reconstruye la matriz solo al cambiar el conjunto de
árboles activos, de modo que el volumen de consultas cae varios órdenes de magnitud
respecto de la etapa de desarrollo y componentes hoy descartados por costo admiten
reevaluarse bajo ese régimen. La restricción RE-04 acota esa reevaluación con un
antecedente propio del proyecto: las fotografías del sistema anterior son hoy
inaccesibles porque el plan de facturación del servicio que las alojaba se cerró, de
modo que la dependencia de un servicio facturado debe limitarse a componentes cuya
pérdida no comprometa los datos.
...
\begin{itemize}[label=--]
  ...
  \item \textbf{RoutingSolution}: resultado de un \texttt{OptimizationJob} para una estrategia de partición dada (campo \texttt{strategy}). Almacena el número de rutas $k$, el tiempo total de desplazamiento, el puntaje de balance temporal, las métricas espaciales de la solución (suma de radios máximos, solapamiento total y por ruta, e \textit{IoU} (intersección sobre unión) del peor par de rutas) y el desglose de tiempos por fase del \textit{pipeline} (campo \texttt{timing}). La relación \texttt{participants} registra los censadores que participan de la solución y alimenta el cálculo de carga acumulada por censador (Sección~\ref{sec:ui}).
...
árboles importados desde CSV, que no tienen identificador externo. El orden de visita
de cada ruta se deriva de \texttt{RouteStop.sequence}; no existe un campo redundante
de secuencia en \texttt{Route}.
...
enfoques sobre un mismo conjunto de árboles. Con el fin de gobernar cuál de estas soluciones
...
La Ilustración~\ref{fig:er} presenta el modelo entidad-relación simplificado. Se omiten
...
Dos aristas del diagrama no son obvias a primera vista. La doble ruta
\texttt{RouteStop} $\to$ \texttt{TreeObservation} y \texttt{Tree} $\to$
\texttt{TreeObservation} es lo que permite que una observación de re-censo
sobreviva al plan de rutas que la originó, incluso si esa ruta se reemplaza. La
cardinalidad 1:1 entre \texttt{Dataset} y \texttt{DistanceMatrix} refleja que la
matriz de costos se cachea una vez por conjunto de árboles activos, no una vez
por optimización.
...
de los árboles a censar. Con el propósito de modelar rutas abiertas ---sin punto de inicio ni de
...
de duración inferior a $T_{\min}$, y las presenta en la solución de producción
...
omitir un nodo no es infactible sino costoso. A su vez, la dimensión acumulativa con capacidad
...
---la mitad del tope, dado que cada petición fragmentada transporta simultáneamente
las coordenadas del bloque origen y las del bloque destino---. El número de solicitudes crece
de forma cuadrática con el número de bloques, lo que domina el costo de la primera
construcción de la matriz para \textit{datasets} grandes
(Capítulo~\ref{cap:resultados}). Un tope adicional de $2\,000$ árboles acota la
dimensión de la matriz ---cuyo almacenamiento en JSON crece como $O(n^2)$ y es del
...
cómputo por el número de intentos y resulta frágil, dado que una corrida infactible
no distingue entre «faltan vehículos» y «el modelo no admite solución».
...
$b$ es un margen de holgura de $5$ vehículos, que absorbe la subestimación del
estimador de vecino más cercano sin ampliar el espacio de búsqueda de forma
apreciable ---es fijo y pequeño frente al techo de $N_{\max}=36$ que la
Ecuación~\ref{eq:nmax} produce sobre la instancia de referencia---; el mínimo con
$n$ impide que el techo supere el caso degenerado de un árbol por ruta.

$N_{\max}$ es una cota superior, no una
...
lo obligan. El número de rutas de la solución final es, en la práctica, muy inferior
al techo: sobre los $1\,607$ árboles de la evaluación, con un techo de
$N_{\max}=36$ vehículos, el \textit{solver} activa $25$ rutas
(Capítulo~\ref{cap:resultados}).
...
este caso la Ecuación~\ref{eq:nmax} se evalúa con margen nulo ($b=0$), dado que aquí
...
La misma invocación comprueba la factibilidad de la configuración antes de crear el
trabajo, mediante la función \texttt{check\_config}. Esta distingue dos categorías:
bloqueos, que son condiciones que garantizan infactibilidad (tiempo de servicio mayor
que $T_{\max}$, conjunto de árboles insuficiente), y advertencias, que son condiciones
de riesgo pero no determinantes (régimen de relleno activado, trabajo total muy por
debajo del piso de jornada). El \textit{endpoint} \texttt{GET /api/optimization/estimate/}
devuelve ambas categorías junto a la estimación de flota, de modo que el administrador
puede detectar configuraciones problemáticas antes de lanzar la optimización.
...
propaga como parte del resultado: el conteo en el campo \texttt{dropped\_trees} y el
conjunto concreto en la relación \texttt{dropped} de la \texttt{RoutingSolution},
de modo que el administrador sabe exactamente qué árboles quedaron fuera del plan y
puede tratarlos como excepciones. El Capítulo~\ref{cap:resultados}
...
decisión binaria adicional que amplía el vecindario que el GLS debe explorar.
Si ello reduce la calidad alcanzable dentro del mismo presupuesto de tiempo se
asume como limitación del trabajo (Sección~\ref{sec:limitaciones}).
...
(Sección~\ref{sec:comparacion-estrategias}). El sistema implementa por ello tres
estrategias de partición, registradas en el campo \texttt{strategy} del trabajo de
optimización. La estrategia \texttt{global} reproduce el comportamiento de la
formulación anterior sin cambios y constituye la línea base.

Las tres estrategias no se ofrecen como una elección del administrador. Un valor
adicional del campo, \texttt{compare}, ejecuta las tres sobre el mismo conjunto de
árboles y persiste una \texttt{RoutingSolution} independiente por estrategia, lo que
permite contrastarlas bajo una misma matriz de costos y un mismo presupuesto de
\textit{solver}; es el modo con que se produjeron las comparaciones del
Capítulo~\ref{cap:resultados} y queda reservado a ese uso experimental. El
lanzamiento ordinario despliega en cambio un abanico fijo de combinaciones, descrito
en la Sección~\ref{sec:abanico}, del que \texttt{cluster\_first} está excluida: el
Capítulo~\ref{cap:resultados} la descarta con datos ---deja árboles fuera de la
solución y degrada el balance--- y mantenerla en el abanico de producción gastaría
un tercio del cómputo en una candidata que el criterio de selección nunca elige.
Permanece accesible por la vía experimental, no por la del administrador.
...
\begin{itemize}[label=--]
  ...
  \item \textbf{\texttt{cluster\_first}}: enfoque \textit{cluster-first,
        route-second} \citep{fisher1981generalized}: los árboles se agrupan en $k$
        clústeres mediante el algoritmo de Lloyd \citep{lloyd1982least} con
        inicialización \textit{k-means++} \citep{arthur2007kmeans}; el número de
        grupos $k$ se estima a partir de la carga de trabajo por árbol sobre el
...
decenas de árboles en las configuraciones operacionales, el costo es del orden de
milisegundos, tres órdenes de magnitud bajo el presupuesto del \textit{solver}; la
Sección~\ref{sec:comparacion-greedy} lo cuantifica sobre la instancia de evaluación.
...
Los coeficientes de la función objetivo no admiten un único valor correcto: cada
ajuste compra una propiedad de la solución al precio de otra, y qué propiedad
conviene comprar depende de la densidad del \textit{dataset} y de la holgura de la
jornada (Capítulo~\ref{cap:resultados}). El sistema resuelve esa indeterminación sin
trasladarla al administrador: en lugar de exponer los coeficientes como parámetros,
define tres perfiles de precios cerrados, con nombre y descripción en el lenguaje
del usuario, y los ejecuta todos.

\begin{itemize}[label=--]
  \item \textbf{Equilibrada} (\texttt{default}): los precios de producción tal como
        se describieron en las secciones anteriores. Es el punto de partida y la
        combinación de control.
  \item \textbf{Rutas más parejas} (\texttt{temporal\_span\_100}): activa el costo
        sobre el \textit{span} de la dimensión temporal con coeficiente $100$, lo
        que iguala la duración entre rutas a costa de caminata adicional.
  \item \textbf{Menos zigzag} (\texttt{arc\_linear\_30}): eleva a $30$ el peso
        lineal del arco, lo que penaliza los tramos largos y compacta el trazado
        cuando sobra tiempo de desplazamiento.
\end{itemize}

Una única solicitud de optimización despliega el producto cartesiano de los tres
perfiles por las dos estrategias de producción ---\texttt{global} y
\texttt{spatial\_term}, con \texttt{cluster\_first} excluida
(Sección~\ref{sec:estrategias})---, es decir, seis trabajos que comparten la misma
\texttt{RoutingConfig} y, por tanto, la misma matriz de costos en caché. El costo
marginal de las cinco combinaciones adicionales es el del \textit{solver}, no el de
OSRM, que es la fase cara de la primera construcción
(Sección~\ref{sec:matriz}). Las seis soluciones resultantes se ordenan y se presentan
al administrador como candidatas comparables (Sección~\ref{sec:ui}). La
Ilustración~\ref{fig:abanico} resume el despliegue del abanico y el criterio
lexicográfico que ordena sus candidatas.
...
\begin{figure}[H]
  ...
  \end{tikzpicture}
  \caption[Abanico de producción y criterio de selección]{Abanico de producción: tres
  perfiles de precios por dos estrategias de partición dan seis candidatas que
  comparten configuración y matriz de costos. El criterio lexicográfico las ordena
  aplicando los cinco desempates en secuencia; la primera se marca como recomendada,
  salvo empate técnico con el par de control, y las restantes se informan con su costo
  adicional en caminata.}
  \label{fig:abanico}
\end{figure}
...
\begin{itemize}[label=--]
  \item \texttt{legacy\_app} (esquema \texttt{arbocensus\_app\_*}): la más antigua, base
        ...
        código QR, descartando las lecturas sin coordenada, las que caen fuera del
        rango de Chile continental ---con lo que se eliminan las lecturas de GPS
        fallidas, que la aplicación previa registraba como coordenada nula--- y los
        códigos QR compuestos solo de ceros. Se emplea la
        mediana y no el promedio por su robustez frente a lecturas de GPS atípicas. De
        este modo se obtienen $12\,941$ árboles con posición utilizable, sobre un total
        de $17\,208$ muestras.
...
Ambas fuentes se solapan geográficamente, por lo que se descarta el árbol de
\texttt{legacy\_app} que se encuentre a $5$\,m o menos de uno de \texttt{legacy\_api}
---se conserva este último, que aporta especie y área---. El explorador expone así
$13\,319$ árboles importables: $429$ de \texttt{legacy\_api} y $12\,890$ de
\texttt{legacy\_app}. Ese es el universo disponible; el \textit{dataset} de
$n = 1\,607$ árboles con que se evalúa el sistema
(Capítulo~\ref{cap:resultados}) es una instancia congelada de ese universo, elegida
por reproducibilidad, y no su totalidad.

El acceso a las bases legadas es estrictamente de solo
...
La decisión de diseño central de este módulo es que la importación no crea
un \textit{dataset} de forma inmediata. La primera versión del importador de esta
iteración traía áreas completas a ciegas, lo que resultó inadecuado en la
práctica: el usuario no sabe, antes de
...
divide por ello en dos etapas: primero explorar y seleccionar, y solo después
materializar el \textit{dataset} con la selección confirmada.
...
número de árboles y su polígono. La fuente \texttt{legacy\_api} almacena las áreas
como listas de vértices y no como geometrías, por lo que el polígono se reconstruye
...
\textit{dataset} y la lista de árboles seleccionados. El contrato
...
El explorador admite además filtrar el arbolado por estado (\texttt{already\_imported}
o disponible) y por fuente de origen, y permite seleccionar un árbol que ya fue
importado en otro \textit{dataset}, de modo que una misma posición geográfica puede
integrarse en campañas distintas cuando el territorio se solapa.
...
\begin{enumerate}
  ...
  \item \textbf{Por región trazada}: el administrador traza sobre el mapa la región
        que quiere censar y se agregan a la selección todos los árboles contenidos en
        ella, con independencia del área a la que pertenezcan. La región se traza de
        dos maneras: un polígono arbitrario, marcando sus vértices con clics
        sucesivos y cerrándolo sobre el primero, o un rectángulo, arrastrando el
        cursor entre dos esquinas. Ambas producen la misma entidad ---un anillo de
        vértices---, de modo que un rectángulo recién trazado se edita después como
        cualquier polígono: cada vértice se arrastra para moverlo, el punto medio de
        cada lado se pulsa para insertar un vértice nuevo y el clic derecho sobre un
        vértice lo elimina. La pertenencia de cada árbol se reevalúa por
        punto-en-polígono al soltar el vértice, y no de forma continua durante el
        arrastre, para no recorrer todo el arbolado legado en cada cuadro de
        animación. Pueden coexistir varias regiones, cuya unión constituye la
        selección.
...
deselección manual de cualquier árbol, de forma que los tres mecanismos
...
\capturaui{ui-legacy-import.png}{Mapa de selección de arbolado legado}{Mapa de selección de arbolado legado (\texttt{/admin/datasets/legacy-import}): un polígono editable sobre el arbolado disponible, marcadores coloreados por estado (disponible, seleccionado, ya importado) y panel lateral con la selección acumulada.}{fig:ui_legacy}

El color de cada marcador codifica el estado del árbol en el sistema: verde para disponible, azul para seleccionado en la sesión actual y gris para ya importado en otro \textit{dataset}. El panel lateral lista la selección acumulada ---126 árboles en la sesión de la ilustración--- y permite revisarla árbol por árbol antes de confirmar la creación del \textit{dataset}.
...
valor del catastro está en la comparación del estado de un árbol entre campañas
sucesivas. El modelo de datos separa por ello dos conceptos que suelen confundirse.
El \texttt{RouteStop} es una unidad de trabajo: pertenece a una
...
\texttt{TreeObservation}, en cambio, es un hecho histórico: registra el
...
documental de la observación ---sin ella el estado declarado no es
...
acceso bloqueado u otro) y la fotografía no se exige, dado que el motivo típico
...
El vínculo entre una observación legada y su árbol difiere según la fuente, debido a que
los esquemas de origen no comparten estructura: en \texttt{legacy\_api} las muestras
...
históricas, se conserva su URL (campo \texttt{photo\_url}) y no el archivo, dado que
...
apuntan a nada; en \texttt{legacy\_api} basta con que la referencia no esté vacía,
dado que esa fuente solo almacena URL remotas.
...
La identidad persistente del árbol entre campañas distintas es el par
(\texttt{source}, \texttt{external\_id}), que \texttt{Tree} conserva en toda
importación. El identificador de fila es local al \textit{dataset}, de modo que la
consulta del historial entre campañas se resuelve por la clave natural y no por el
...
\begin{itemize}[label=--]
  \item \textbf{Autenticación} (\texttt{/api/auth/}): obtención y renovación de \textit{tokens} \textit{JSON Web Token} (JWT) mediante \textit{djangorestframework-simplejwt}, consulta del usuario en sesión y administración de las cuentas de censadores.
  ...
  \item \textbf{Optimización} (\texttt{/api/optimization/}): cubre el ciclo completo de un trabajo de optimización. Estima la flota y comprueba la factibilidad de la configuración (bloqueos y advertencias) antes de optimizar; crea los trabajos ---una sola solicitud genera los seis del abanico de producción (Sección~\ref{sec:abanico}) y responde con la lista completa---; consulta su estado (para \textit{polling} desde el \textit{frontend}); expone las métricas de cada \texttt{RoutingSolution} ordenadas por el criterio de recomendación; y publica o despublica la solución vigente por \textit{dataset}.
  \item \textbf{Rutas} (\texttt{/api/routes/}): listado de rutas con métricas, exportación \textit{GeoJSON} para visualización cartográfica, asignación de censadores, carga acumulada por censador con sugerencia de asignación voraz, consulta de la ruta propia del censador, y registro de visitas y omisiones en terreno con su observación asociada.
\end{itemize}

El sistema implementa dos roles: Administrador (acceso completo) y Censador
(lectura de las rutas propias del plan vigente y registro de visitas). La autorización se resuelve por rol mediante los permisos propios
...
el \hyperref[anx:api]{Anexo 1} contiene el inventario completo con sus permisos y
respuestas.

\begin{table}[H]
  \centering
  \caption{Grupos de recursos de la API REST. El Anexo 1 contiene el inventario completo con permisos y respuestas.}
  \label{tab:endpoints}
  \small
  \begin{tabularx}{\textwidth}{@{}l l X@{}}
    ...
    \midrule
    Autenticación & \texttt{/api/auth/} & Tokens JWT, consulta del usuario en sesión y gestión de cuentas de censadores. \\
    \textit{Datasets} & \texttt{/api/datasets/} & Importación CSV/JSON y desde fuentes legadas, exploración, exportación \textit{GeoJSON} e historial de observaciones. \\
    Optimización & \texttt{/api/optimization/} & Estimación de flota y comprobación de factibilidad, creación de trabajos, \textit{polling} de estado, métricas de soluciones y publicación del plan vigente. \\
    Rutas & \texttt{/api/routes/} & Listado con métricas, exportación \textit{GeoJSON}, asignación de censadores, carga acumulada por censador con sugerencia de asignación, ruta propia del censador y registro de visitas y omisiones. \\
    \bottomrule
  \end{tabularx}
\end{table}

Los grupos \textit{Datasets} y Optimización son exclusivos del rol Administrador,
al igual que la administración de cuentas dentro del grupo Autenticación; el resto
de ese grupo ---obtención y renovación del \textit{token} y consulta de la sesión propia---
lo utilizan ambos roles. Rutas es el único grupo cuyas operaciones de negocio
comparten los dos, con el censador restringido a su propia ruta vigente.
...
\begin{enumerate}
  ...
  \item \textbf{Constitución del \textit{dataset}.} Importa un archivo CSV o selecciona el arbolado sobre el mapa de fuentes legadas (Sección~\ref{sec:legacy}); en ambos casos el resultado se visualiza como puntos sobre el mapa (\texttt{DatasetMap}). El listado de \textit{datasets} disponibles aparece en la Ilustración~\ref{fig:ui_datasets} del \hyperref[anx:capturas]{Anexo 6}.
  \item \textbf{Configuración y lanzamiento.} Configura $T_{\min}$, $T_{\max}$ y el tiempo de servicio por árbol, y lanza el trabajo. El formulario no expone ni la estrategia ni los coeficientes del objetivo: una sola solicitud despliega el abanico de seis combinaciones descrito en la Sección~\ref{sec:abanico}. El \textit{endpoint} \texttt{GET /api/optimization/estimate/} se consulta a medida que el administrador escribe y comprueba la factibilidad de la configuración antes de que se cree el trabajo: las advertencias se muestran junto al formulario y los bloqueos inhabilitan el botón de lanzamiento.
  \item \textbf{Monitoreo.} \textit{React Query} consulta el estado del trabajo cada 3 segundos mientras se encuentra en \texttt{queued} o \texttt{running}, y detiene las consultas al alcanzar un estado terminal o superar 15 minutos; el registro permanece en \texttt{running} si el \textit{worker} muere (Sección~\ref{sec:limitaciones}).
  \item \textbf{Presentación, recomendación y publicación.} Las rutas resultantes se presentan como polilíneas coloreadas sobre la red peatonal real. El componente \texttt{SolutionCandidatesList} lista las seis soluciones como candidatas a plan, ordenadas de mejor a peor por el criterio lexicográfico que implementa \texttt{order\_by\_criterion}. El orden aplica en secuencia: sin árboles fuera de ruta, luego sin rutas degeneradas, luego puntaje de balance no inferior a $0{,}60$, luego menor tiempo de desplazamiento y menor número de rutas. Sobre ese orden se aplica una última regla de estabilidad: si el par de control ---perfil \texttt{default} con estrategia \texttt{spatial\_term}--- supera las mismas compuertas que el primero y su desplazamiento no se aparta de él más de un $3$\,\%, se recomienda el control, con el fin de que un empate dentro del ruido del \textit{solver} no cambie la recomendación entre corridas. La candidata así elegida se marca como recomendada y las demás informan cuánta caminata adicional cuestan respecto de ella; una candidata cuyo desplazamiento difiere de la recomendada en menos del $3$\,\% se rotula como empate técnico, de modo que el administrador distingue una ventaja real de una diferencia dentro del ruido del \textit{solver}. La recomendación no es vinculante: el administrador publica la candidata que estime conveniente. Publicado el plan, \texttt{suggest\_assignment} propone una asignación de rutas a censadores que reduce el déficit de carga acumulado entre campañas, y el administrador la acepta o la modifica ruta por ruta.
\end{enumerate}

La Ilustración~\ref{fig:ui_optimization} corresponde al panel de optimización del \textit{dataset}.

\capturaui{ui-optimization.png}{Detalle de un \textit{dataset}: panel de optimización}{Detalle de un \textit{dataset} (\texttt{/admin/datasets/\{id\}}), panel de optimización: el mapa muestra la solución recomendada sobre la red peatonal y el panel lateral lista las candidatas a plan ordenadas por el criterio de selección, cada una con sus métricas y con la caminata adicional que cuesta respecto de la recomendada. El formulario de parámetros temporales encabeza el mismo panel.}{fig:ui_optimization}

El panel derecho concentra la configuración de parámetros ($T_{\min}$, $T_{\max}$ y tiempo de servicio) y el estado de la última optimización; el clic sobre un marcador del mapa despliega el historial de observaciones de ese árbol, lo que permite al administrador verificar si ya fue censado antes de lanzar la optimización.

El panel de asignación aparece en la Ilustración~\ref{fig:ui_routes}.

\capturaui{ui-routes.png}{Detalle de un \textit{dataset}: panel de asignación}{El mismo detalle en su panel de asignación: el plan publicado como polilíneas coloreadas sobre la red peatonal y, por cada ruta, sus paradas, su duración estimada y el censador asignado.}{fig:ui_routes}

Cada candidata resume tres métricas ---número de rutas, tiempo de caminata y puntaje de balance--- y destaca en cuáles de ellas es la mejor de la corrida; los árboles que quedan fuera de ruta, las rutas degeneradas y el balance bajo el umbral no se presentan como cifras sino como advertencias en lenguaje llano, porque son condiciones que descalifican a la candidata y no magnitudes que el administrador deba ponderar. El panel derecho materializa la asignación de cada ruta a un censador. Como dos corridas con parámetros temporales distintos no son comparables entre sí, la lista muestra una sola corrida a la vez y un selector permite alternar entre ellas.

Publicado el plan, el administrador supervisa el avance del censo desde la pestaña
de progreso: el tablero resume el porcentaje de árboles censados, omitidos y
pendientes, los sitúa sobre el mapa según su estado y desglosa el avance por ruta y
por censador, de modo que se identifican qué rutas van retrasadas. Esta vista
realiza RF-19 y RF-20; la ilustración correspondiente se incluye en el
...
Los marcadores numerados sobre el mapa reflejan el orden de visita derivado de \texttt{RouteStop.sequence}; las paradas posteriores a la activa aparecen bloqueadas hasta que el censador complete la anterior, lo que impone el orden secuencial descrito en la Sección~\ref{sec:ui}. El botón «Registrar de todos modos» habilita el registro cuando la señal GPS no alcanza el radio de proximidad de $30$\,m.
...
\begin{xltabular}{\textwidth}{@{}l l X@{}}
  ...
  \bottomrule
  \endlastfoot
  Pruebas de \textit{backend} & 524 funciones en 47 archivos & \textit{Solver} y \textit{pipeline} de optimización, estimación de flota, construcción y caché de la matriz de costos, importadores de CSV y de ambas fuentes legadas, ciclo de publicación, autorización por rol y registro de visitas y omisiones. \\
  Pruebas de \textit{frontend} & 304 casos en 44 archivos & Componentes de la interfaz, cliente de API, cola \textit{offline}, utilidades de selección cartográfica y de progreso. \\
...
Corresponde acotar el alcance de esta verificación. No existen pruebas automatizadas
de extremo a extremo: los flujos completos de administrador y de censador se comprueban por partes, nunca
...
memoria no reporta una cifra de cobertura. Sobre todo, lo que se verifica es que
...
A esta verificación del sistema se suma un compromiso distinto: la reproducibilidad
de las cifras experimentales que reporta el Capítulo~\ref{cap:resultados}. La
búsqueda local del \textit{solver} se corta por reloj de pared y la biblioteca no
permite fijar una semilla ---el valor que determina el punto de partida de un
generador de números pseudoaleatorios y que, al repetirse, permite reconstruir una
corrida idéntica---, de modo que dos ejecuciones del mismo problema pueden diferir en
los segundos finales de búsqueda. Ambas condiciones se registran entre las
restricciones de la Sección~\ref{sec:restricciones}. El compromiso es que un tercero
pueda regenerar las cifras publicadas desde el mismo comando de gestión y la misma
instancia de datos congelada y versionada, no que el \textit{solver} sea
determinista; las métricas por ruta se re-derivan con \texttt{route\_audit} sobre esa
misma instancia. Reproducir una cifra tampoco significa que la solución sea óptima:
acredita el procedimiento, no la calidad del plan, que es lo que miden los atributos
de la Sección~\ref{sec:atributos-calidad}.
