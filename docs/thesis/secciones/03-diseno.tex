...
\begin{itemize}
  ...
  \item \textbf{Optimización} (\texttt{/api/optimization/}): estimación de flota y comprobación de factibilidad de la configuración (bloqueos y advertencias) previa a la optimización, creación de trabajos, consulta de estado (para \textit{polling} desde el \textit{frontend}), acceso a las métricas de cada \texttt{RoutingSolution} y publicación o despublicación de la solución vigente por \textit{dataset}.
...
\begin{enumerate}
  ...
  \item \textbf{Configuración y lanzamiento.} Configura $T_{\min}$, $T_{\max}$ y el tiempo de servicio por árbol, elige la estrategia ---o la opción \textit{«Comparar las 3»}--- y lanza el trabajo. El \textit{endpoint} \texttt{GET /api/optimization/estimate/} comprueba la factibilidad de la configuración (bloqueos y advertencias) antes de que se cree el trabajo.
  ...
  \item \textbf{Presentación, recomendación y publicación.} Las rutas resultantes se presentan como polilíneas coloreadas sobre la red peatonal real; \texttt{StrategyTabs} permite contrastar las soluciones de cada estrategia. La función \texttt{pick\_recommended} señala automáticamente la solución preferible según criterio lexicográfico; \texttt{suggest\_assignment} propone la asignación de rutas a censadores que minimiza el déficit de carga acumulado entre campañas. El administrador publica la solución elegida y asigna las rutas. El tablero de avance del censo se muestra en la Ilustración~\ref{fig:ui_progress} del Anexo~\ref{anx:capturas}.
\end{enumerate}
...
El panel derecho concentra la configuración de parámetros ($T_{\min}$, $T_{\max}$, tiempo de servicio y estrategia) y el estado de la última optimización; el clic sobre un marcador del mapa despliega el historial de observaciones de ese árbol, lo que permite al administrador verificar si ya fue censado antes de lanzar la optimización.

La pestaña de asignación con las tres estrategias comparables aparece en la Ilustración~\ref{fig:ui_routes}.
...
Las tres pestañas superiores resumen las métricas de cada estrategia ---número de rutas y tiempo total de jornada--- lo que permite al administrador seleccionar la solución que publicará; el panel derecho materializa la asignación de cada ruta a un censador antes de publicar el plan.
