...
\begin{itemize}
  ...
  \item \textbf{RoutingSolution}: resultado de un \texttt{OptimizationJob} para una estrategia de partición dada (campo \texttt{strategy}). Almacena el número de rutas $k$, el tiempo total de desplazamiento, el puntaje de balance temporal, las métricas espaciales de la solución (suma de radios máximos, solapamiento total y por ruta, e \textit{IoU} (intersección sobre unión) del peor par de rutas) y el desglose de tiempos por fase del \textit{pipeline} (campo \texttt{timing}).
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}l l X@{}}
    ...
    Optimización & \texttt{/api/optimization/} & Estimación de flota, creación de trabajos, \textit{polling} de estado, métricas de soluciones y publicación del plan vigente. \\
    Rutas & \texttt{/api/routes/} & Listado con métricas, exportación \textit{GeoJSON}, asignación de censadores, ruta propia del censador y registro de visitas y omisiones. \\
    \bottomrule
  \end{tabularx}
\end{table}
