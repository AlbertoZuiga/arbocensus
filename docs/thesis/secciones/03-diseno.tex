...
\begin{enumerate}
  \item Costo. Bajo un volumen elevado de ejecuciones, un componente tarificado por consulta
        convierte cualquier precio unitario en un gasto difícil de acotar; el criterio fue mantener
        en cero el gasto de la etapa de desarrollo, sin excluir adoptar más adelante un componente
        de pago cuyo aporte al producto en operación justifique su costo recurrente.
...
esas decisiones es que ninguna se resolvió por rendimiento aislado del componente: cuando existía
...
inferior a $T_{\min}$.\footnote{El Capítulo~\ref{cap:resultados} las observa en las áreas reales
de baja carga, no en la instancia de referencia.}
...
registrar, y esa variación es la que $s$ traslada al modelo.
...
\begin{enumerate}
  \item \texttt{global}: el \textit{Open mTSP} único descrito en la Sección~\ref{sec:solver}, sin
        término espacial ni partición previa del territorio. Constituye la línea base.
...
de magnitud bajo el presupuesto del \textit{solver}.\footnote{Un ciclo de evaluación anterior lo
rechazó porque empeoraba la métrica de auto-cruces medida sobre cuerdas rectas; al corregirse el
instrumento se estableció que mejora simultáneamente el tiempo de viaje y la geometría real, lo que
revirtió el rechazo (Sección~\ref{sec:discusion-calidad}).}
...
campo \texttt{source} del modelo \texttt{Tree}.\footnote{Ver \hyperref[anx:modelo-legado]{Anexo 7}.}
...
importables: $429$ de \texttt{legacy\_api} y $12\,890$ de
\texttt{legacy\_app}.\footnote{El \textit{dataset} de $n = 1\,607$ árboles con que se evalúa el
sistema (Capítulo~\ref{cap:resultados}) es una instancia congelada de ese universo.} El acceso a
las bases legadas es estrictamente de solo lectura
(RE-03).
...
registra como una observación más en esa serie.
...
\caption{Grupos de recursos de la API REST.}
  ...
  \begin{tabularx}{\textwidth}{@{}l l X@{}}
    ...
    \textit{Datasets} &
      \texttt{/api/datasets/} &
      Importación CSV/JSON y desde fuentes legadas, exploración, exportación \textit{GeoJSON},
      historial de observaciones, edición de metadatos y desactivación de árboles. \\
    Optimización &
...
que se ejecuta en cada integración de cambios: 533 funciones de prueba de \textit{backend} en 47
archivos y 316 casos de \textit{frontend} en 44, acompañados de análisis estático
