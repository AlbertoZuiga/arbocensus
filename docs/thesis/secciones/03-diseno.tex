...
milisegundos: la Tabla~\ref{tab:comparativa} reporta $0{,}09$\,s para el
\textit{greedy} con post-proceso frente a $0{,}007$\,s sin él, lo que sitúa el
post-proceso en aproximadamente $0{,}08$\,s sobre los $1\,607$ árboles de la
evaluación, tres órdenes de magnitud bajo el presupuesto del \textit{solver}
(Capítulo~\ref{cap:resultados}).
...
\begin{enumerate}
  ...
  \item \textbf{Por figura dibujada}: el administrador traza sobre el mapa la región
        que quiere censar y se agregan a la selección todos los árboles contenidos en
        ella, con independencia del área a la que pertenezcan. La figura se traza de
        ...
        animación. Pueden coexistir varias figuras, cuya unión constituye la
        selección.
...
confundir. El \texttt{RouteStop} es una unidad de trabajo: pertenece a una
...
\begin{itemize}
  ...
  \item \textbf{Optimización} (\texttt{/api/optimization/}): estimación de flota y comprobación de factibilidad de la configuración (bloqueos y advertencias) previa a la optimización, creación de trabajos ---una sola solicitud crea los seis del abanico de producción (Sección~\ref{sec:abanico}) y responde con la lista completa---, consulta de estado (para \textit{polling} desde el \textit{frontend}), acceso a las métricas de cada \texttt{RoutingSolution} ordenadas por el criterio de recomendación, y publicación o despublicación de la solución vigente por \textit{dataset}.
