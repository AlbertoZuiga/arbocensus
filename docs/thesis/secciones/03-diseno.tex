...
La Tabla~\ref{tab:stack-decisions} registra las alternativas evaluadas para cada componente del \textit{stack} y el criterio de elección.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}l l X@{}}
    \toprule
    Decisión & Alternativa considerada & Criterio de elección \\
    \midrule
    Django + GeoDjango & FastAPI (iteración anterior) & GeoDjango aporta soporte nativo de PostGIS y operaciones espaciales vía el ORM; la iteración anterior del proyecto usaba FastAPI, que no tiene equivalente integrado. \\[4pt]
    Celery + Redis & Ejecución síncrona & La construcción de la matriz de costos y la ejecución del \textit{solver} pueden tardar varios minutos; resolverlos en el ciclo de la petición HTTP excede los tiempos de respuesta aceptables de un servidor web. \\[4pt]
    OSRM & Valhalla, GraphHopper & Disponibilidad de perfil peatonal preprocesable localmente a partir de datos de \textit{OpenStreetMap}; elimina costos de API y dependencias de red durante la optimización. \\[4pt]
    \textit{Leaflet} & \textit{MapLibre GL} & API madura con integración directa en \textit{React} sin requerir un servidor de teselas vectoriales propio, suficiente para el alcance de visualización del producto. \\
    \bottomrule
  \end{tabularx}
\end{table}

En los cuatro casos el criterio dominante fue la integración con el resto del
\textit{stack} y no el rendimiento aislado del componente.
