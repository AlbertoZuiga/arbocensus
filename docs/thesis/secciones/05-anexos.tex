...
\begin{center}
  {\bfseries ÍNDICE DE ANEXOS}
\end{center}
\vspace{6pt}
\noindent
\begin{tabularx}{\textwidth}{@{}X r@{}}
  Anexo 1. Documentación de la API REST & \pageref{anx:api} \\
  Anexo 2. Comando de demostración (\texttt{seed\_demo}) & \pageref{anx:demo} \\
  Anexo 3. Comandos reproducibles de los experimentos & \pageref{anx:comandos} \\
  Anexo 4. Grilla de variantes y auditoría de robustez & \pageref{anx:grilla-variantes} \\
  Anexo 5. Trazabilidad de requisitos al diseño & \pageref{anx:trazabilidad} \\
  Anexo 6. Capturas complementarias de la interfaz & \pageref{anx:capturas} \\
  Anexo 7. Modelo de datos de las bases legadas & \pageref{anx:modelo-legado} \\
\end{tabularx}
\newpage
...
Público (sin autenticación), Autenticado (cualquier usuario con
sesión válida), Admin (\texttt{IsAdminRole}) y Censador
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} p{1.9cm} X@{}}
    ...
    \texttt{GET \slash api\slash auth\slash surveyors\slash } &
      Admin &
      Censadores registrados, para poblar el selector de asignación de rutas. \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} p{1.9cm} X@{}}
    ...
    \texttt{GET \slash api\slash optimization\slash solutions\slash \{id\}\slash } &
      ...
      balance, métricas espaciales y tiempos por fase del \textit{pipeline}. El censador solo
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} p{1.9cm} X@{}}
    ...
    \texttt{GET \slash api\slash routes\slash } &
      ...
      \texttt{solution\_id}. El censador solo ve las rutas asignadas a él. \\
    ...
    \texttt{PATCH \slash api\slash routes\slash \{id\}\slash assign\slash } &
      Admin &
      Asigna o desasigna un censador. Solo se permite sobre rutas de la solución publicada. \\
...
la descripción de la suite de instancias congeladas sobre la que se ejecutó el barrido
de configuración y la descripción completa de los cuatro defectos de instrumentación
detectados en la serie experimental.
...
\begin{table}[H]
  \centering
\caption{Grilla de seis variantes sobre el \textit{dataset} real ($n=1\,607$; media de 3 semillas;
    estrategia \texttt{global}; límite de \textit{solver} de 180\,s; código previo a la
    corrección del redondeo del tránsito). El total del \textit{pipeline} equivale al tiempo
de \texttt{solve} más $\approx$0{,}6\,s de sobrecosto fijo.}
...
(Sección~\ref{sec:discusion-calidad}) están versionadas en
\texttt{docs/experiments/instances/} y se describen en la
Tabla~\ref{tab:suite-instancias}. La batería de crecimiento se construye sobre los
$12\,946$ árboles georreferenciados de \texttt{legacy\_app}, ordenados por distancia de
\textit{haversine} a un centroide fijo: cada instancia es prefijo exacto de la siguiente,
de modo que el crecimiento es puramente espacial y no una muestra distinta. Las dos
variantes dispersas toman uno de cada dos y uno de cada cuatro sobre los primeros
$1\,000$, con lo que conservan la extensión y reducen la densidad a la mitad y a un
cuarto: aíslan el efecto de la densidad del efecto del tamaño.
...
\textit{datasets} e importación de archivos (\texttt{/admin/datasets}): tabla con nombre, número
de árboles y fecha de importación; los botones superiores dan acceso a las dos rutas de
creación.}{fig:ui_datasets}
...
(\texttt{/admin/datasets/\{id\}/progress}): porcentaje de árboles censados, omitidos y
pendientes; mapa coloreado por el estado de cada árbol; y desglose del avance por ruta y por
censador.}{fig:ui_progress}
...
\phantomsection
\section*{Anexo 7. Modelo de datos de las bases legadas}
\addcontentsline{toc}{section}{Anexo 7. Modelo de datos de las bases legadas}
