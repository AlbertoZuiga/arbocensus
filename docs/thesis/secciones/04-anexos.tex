...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} p{1.9cm} X@{}}
    ...
    \midrule
    \texttt{POST /api/auth/token/}          & Público      & Par de tokens JWT (\texttt{access}, \texttt{refresh}) a partir de usuario y contraseña. \\
    \texttt{POST /api/auth/token/refresh/}  & Público      & Nuevo token de acceso a partir de un token de refresco válido. \\
    \texttt{GET /api/auth/me/}              & Autenticado  & Usuario en sesión: \texttt{id}, \texttt{username}, \texttt{email} y \texttt{role}. Es la consulta que usa el \textit{frontend} para decidir qué interfaz mostrar. \\
    \texttt{GET /api/auth/surveyors/}       & Admin        & Censadores registrados, para poblar el selector de asignación de rutas. \\
    \texttt{GET /api/auth/users/}           & Admin        & Listado paginado de usuarios. \\
    \texttt{POST /api/auth/users/}          & Admin        & Crea un usuario. La contraseña es de solo escritura y se valida con las reglas de contraseña de Django. \\
    \texttt{GET /api/auth/users/\{id\}/}    & Admin        & Detalle de un usuario. \\
    \texttt{PATCH /api/auth/users/\{id\}/}  & Admin        & Actualiza rol, estado o contraseña. Rechaza que un administrador se degrade o desactive a sí mismo, y que se elimine el último administrador activo. \\
    \texttt{DELETE /api/auth/users/\{id\}/} & Admin        & Desactivación lógica (\texttt{is\_active = false}); el usuario no se borra, para no arrastrar sus observaciones históricas. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} p{1.9cm} X@{}}
    ...
    Método y ruta & Permiso & Devuelve \\
    \midrule
    \texttt{GET /api/datasets/}                       & Autenticado & Listado paginado de \textit{datasets} con su número de árboles. \\
    \texttt{POST /api/datasets/}                      & Admin       & Crea un \textit{dataset} importando un archivo \texttt{.csv} o \texttt{.json} (\textit{multipart}). Detecta las columnas de coordenadas por nombre (\texttt{lat}/\texttt{latitude}/\texttt{latitud} y \texttt{lon}/\texttt{lng}/\texttt{longitude}/\texttt{longitud}) e infiere el delimitador del CSV. Devuelve el \textit{dataset} creado y \texttt{skipped\_rows}, el número de filas descartadas por coordenada faltante. \\
    \texttt{GET /api/datasets/\{id\}/}                & Autenticado & Detalle de un \textit{dataset}. \\
    \texttt{PATCH /api/datasets/\{id\}/}              & Admin       & Actualiza nombre o descripción. \\
    \texttt{DELETE /api/datasets/\{id\}/}             & Admin       & Elimina el \textit{dataset} y, en cascada, sus árboles y soluciones. \\
    \texttt{GET /api/datasets/\{id\}/trees/}          & Autenticado & Árboles activos del \textit{dataset} como \textit{FeatureCollection} de puntos. \\
    \texttt{GET /api/datasets/legacy/areas/}          & Admin       & Áreas de censo de la fuente legada con su campaña, su número de árboles y su polígono. Responde 503 si la fuente no está configurada. \\
    \texttt{GET /api/datasets/legacy/trees/}          & Admin       & Arbolado de ambas fuentes legadas con su posición, su especie, el área que lo contiene y la marca \texttt{already\_imported}. \\
    \texttt{POST /api/datasets/from-legacy-selection/} & Admin      & Crea un \textit{dataset} en una única transacción a partir de una lista de árboles identificados por el par (\texttt{source}, \texttt{external\_id}), junto con su histórico de observaciones. \\
    \texttt{POST /api/datasets/import-legacy/}        & Admin       & Importa un área completa (\texttt{area\_id}) o la fuente entera (\texttt{area\_id} con valor \texttt{all}) sin selección previa. Atajo programático sin interfaz asociada. \\
    \texttt{GET /api/datasets/trees/\{id\}/observations/} & Autenticado & Historial completo de observaciones de un árbol, incluidas las importadas de las fuentes legadas. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} p{1.9cm} X@{}}
    ...
    Método y ruta & Permiso & Devuelve \\
    \midrule
    \texttt{GET /api/optimization/estimate/}                 & Admin       & Estimación de flota para un \textit{dataset} (parámetros de consulta \texttt{dataset}, \texttt{min\_route\_time\_sec} y \texttt{service\_time\_sec}). Se evalúa sobre la matriz de costos en caché y no dispara consultas a OSRM: si el \textit{dataset} no tiene matriz calculada, devuelve \texttt{n\_estimated} nulo. \\
    \texttt{POST /api/optimization/jobs/}                    & Admin       & Crea la \texttt{RoutingConfig} ($T_{\min}$, $T_{\max}$, tiempo de servicio) y encola el \texttt{OptimizationJob} en Celery. El campo \texttt{strategy} admite \texttt{global}, \texttt{spatial\_term}, \texttt{cluster\_first} o \texttt{compare} (ejecuta las tres y produce una solución por estrategia). \\
    \texttt{GET /api/optimization/jobs/}                     & Admin       & Historial de trabajos, filtrable por \texttt{dataset}. \\
    \texttt{GET /api/optimization/jobs/\{id\}/}              & Admin       & Estado del trabajo (\texttt{queued}, \texttt{running}, \texttt{completed}, \texttt{failed}), sus métricas, los árboles descartados y el mapa \texttt{solution\_ids} de estrategia a solución. Es el \textit{endpoint} que el \textit{frontend} consulta por \textit{polling}. \\
    \texttt{GET /api/optimization/solutions/\{id\}/}         & Autenticado & Métricas de una solución: número de rutas, tiempo total de desplazamiento, puntaje de balance, métricas espaciales y tiempos por fase del \textit{pipeline}. El censador solo accede a las soluciones que contienen rutas suyas. \\
    \texttt{POST /api/optimization/solutions/\{id\}/publish/}   & Admin    & Publica la solución como plan vigente del \textit{dataset} y despublica atómicamente la anterior. \\
    \texttt{POST /api/optimization/solutions/\{id\}/unpublish/} & Admin    & Retira la solución del plan vigente. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} p{1.9cm} X@{}}
    ...
    Método y ruta & Permiso & Devuelve \\
    \midrule
    \texttt{GET /api/routes/}                    & Autenticado & Rutas con sus tiempos y su avance (visitadas, pendientes, omitidas), filtrables por \texttt{solution\_id}. El censador solo ve las rutas asignadas a él. \\
    \texttt{GET /api/routes/\{id\}/}             & Autenticado & Detalle de una ruta, incluidas sus paradas ordenadas por \texttt{sequence}. \\
    \texttt{GET /api/routes/geojson/}            & Autenticado & Polilíneas de todas las rutas de una solución (\texttt{solution\_id} obligatorio) como \textit{FeatureCollection} de \textit{LineString}, trazadas sobre la red peatonal real que devuelve OSRM. \\
    \texttt{GET /api/routes/\{id\}/path/}        & Autenticado & Trazado peatonal de una sola ruta. \\
    \texttt{PATCH /api/routes/\{id\}/assign/}    & Admin       & Asigna o desasigna un censador. Solo se permite sobre rutas de la solución publicada. \\
    \texttt{GET /api/routes/my-route/}           & Censador    & Rutas propias del plan vigente; excluye por construcción las rutas de soluciones no publicadas. \\
    \texttt{POST /api/routes/stops/\{id\}/visit/} & Censador   & Marca la parada como visitada y crea su \texttt{TreeObservation} (estado, fotografía y notas; \textit{multipart} cuando incluye imagen). Rechaza la visita si alguna parada anterior sigue pendiente. \\
    \texttt{POST /api/routes/stops/\{id\}/skip/}  & Censador   & Omite la parada con un motivo obligatorio y crea la observación correspondiente, derivando su estado del motivo. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\texttt{docs/experiments/}, con su comando, sus parámetros y sus métricas.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}p{5.5cm} X@{}}
    ...
    \texttt{--time-limit <s>}       & Fija el límite de tiempo del solver, en lugar de la heurística del \textit{pipeline} ($\min(30 + 1{,}5\,n,\;180)$ segundos). \\
...
se documenta en \texttt{docs/experiments/profiling-20260711.md} (perfilado por fases) y
\texttt{docs/experiments/route-quality-20260712.md} (comparación de estrategias). Las
cifras que sustentan el Capítulo~\ref{cap:resultados} provienen de los CSV
depositados en ese mismo directorio.
