# Q1 — Sensibilidad a las penalizaciones del solver (test causal de F11.1)

- Fecha: 2026-07-13 (UTC)
- Branch: `penalty-sensitivity` (PR #158), sobre main `c51efad` (que ya incluye Q0, PR #154)
- Objetivo: test causal de **F11.1** ("quedar corto de T_min se arregla caminando", confirmada
  por Q0 en régimen disperso) y del eje del soft upper reformulado por Q0 (F11.2 refutada: el
  soft upper en el midpoint se paga en 25/25 rutas sin bajar ninguna → ¿sirve de algo, o conviene
  apuntarlo a T_max?). Este reporte mide; **no cambia ninguna constante ni default de producción**
...
Idéntico a Q0 (`route-audit-20260713.md` §a), RP2 y R2: `docker-compose.prod.yml`, imágenes target
...
Recrear los datasets (mismos comandos que Q0; los UUID son nuevos porque el volumen es nuevo):
...
full = legacy.create_dataset('Q1 penalty sensitivity - legacy completo (ambas fuentes)', loaded)
...
docker compose -f docker-compose.prod.yml run --rm backend python manage.py seed_demo \
  ...
  --name "Q1 penalty sensitivity - seed_demo disperso n40" --no-optimize
```
...
Las 36 corridas salen del comando `route_audit` con los overrides nuevos del PR #158
...
docker compose -f docker-compose.prod.yml run --rm -e PYTHONUNBUFFERED=1 backend \
  python manage.py route_audit \
    ...
    --csv /results/q1-disp-slp0-tmax-s42.csv \
    --geojson /results/q1-disp-slp0-tmax-s42.geojson
```

La grilla completa (3 penalizaciones × 2 objetivos × 3 semillas, por dataset) se corrió con
`.local/q1-grid.sh <tag> <dataset> <service-time>`; los agregados por corrida se producen con
`.local/q1-aggregate.py`. `--soft-upper-penalty` se dejó en su valor de producción (500) en toda
la grilla: el eje que Q0 dejó abierto es **dónde** se para el soft upper, no cuánto cuesta.

Definiciones (idénticas a Q0, más una): `walk_ratio` = caminata / duración;
**`cobertura del hueco de T_min`** = caminata / max(0, T_min − servicio), promediada sobre las
rutas cuyo servicio no llega a T_min — es la firma de F11.1 (una ruta que "camina para llegar a
...
## b. Grilla del disperso n=40 (el caso patológico de F11, service 300 s)
...
caminata seguiría dominando al servicio**. F11.1 sigue siendo cierta —la duración se detiene justo
...
(Diferencia con Q0: allá el baseline del disperso dio k=5 y walk 0,670 con cobertura 100–105 %;
aquí da k=4, walk 0,661 y cobertura 141 %. Mismo dataset, misma configuración: es una partición
distinta a la que llegó el GLS dentro de los mismos 120 s. La firma de F11.1 —duraciones clavadas
...
El baseline reproduce la referencia publicada y la corrida R1 de Q0: k=25, 0 drops, 0 excesos de
T_max, walk 0,235 (Q0: 0,233), travel 59 372 s (Q0: 58 664 s, dentro de la variación por corte de
...
2. **Bajar la penalización de T_min sin mover el soft upper empeora todo.** slp = 100 abre una ruta
   ...
   caminata total.
3. **Esto responde la hipótesis abierta de Q0** ("con 25 rutas pagando ~1 060 s sobre el midpoint,
...
**Hallazgo lateral, fuera del criterio (relevante para F12/Q1b):** mover el soft upper a T_max con
la penalización de T_min intacta deja el travel casi igual (−1,1 %), k igual (25), 0 drops, 0
excesos… y **baja los auto-cruces del polyline de 76 a 6** (−92 %). El soft upper en el midpoint
...
criterio de Q1 —que se decide por walk_ratio y travel—, pero ataca justo la métrica que la revisión
visual (F12) señaló como la queja real. **Es material para Q1b, no una recalibración aprobada.**
...
**Documentar F11 como trade-off inherente al diseño. No ejecutar Q2** (no tocar `solver.py`).
...
2. El caso patológico de F11 (disperso n=40) resulta **insensible** a las penalizaciones: su
   ...
   duplicando al censo. Esto coincide con la lectura visual de F12 (ese dataset se ve "disperso por
...
Lo que Q1 sí deja como material de tesis (§3.1.4 / limitaciones, Q3):
...
- **Por qué el solver no abre más rutas** (hipótesis abierta de Q0): jerarquía de penalizaciones —
  soft lower 10 000/s domina 20:1 al soft upper 500/s. No es límite del GLS ni compensación del
  travel (§c.3).
- **El soft upper en el midpoint no es inerte, es dominado**: matiza la refutación de F11.2 de Q0.
  Con el soft lower activo se paga en 25/25 rutas sin bajarlas; sin soft lower, empuja de verdad
  (k=32, T̄ 8 449 s).
- **Gancho para Q1b (F12):** `soft_upper_target=tmax` baja los auto-cruces de 76 a 6 con travel y k
  intactos, a costa del balance (0,715). La geometría de las rutas **sí** responde a este eje —
  justo la métrica que la revisión visual señaló. Q1b debería incluirlo en su grilla junto a
  `time_limit` y `FIXED_VEHICLE_COST`, midiendo balance como restricción dura.
