...
limpiamente los dos mundos: n=1607 corre a **0,94** (rutas saturadas de servicio), mientras las tres
áreas reales corren a **0,73 / 0,67 / 0,67** (n=157/72/43) — todas lejos del piso que D necesita para
no meter caminata de relleno. Una campaña real trabaja un área, cuyo tamaño es del orden de decenas o
pocos cientos de árboles, **no** la unión de ~1600; ninguna instancia que de verdad se ejecuta entra
en el régimen saturado. Por eso D no es un default candidato para ningún plan que se despache: es un
resultado **medido sobre la instancia agregada**, no una recalibración lista para producción.

**Recomendación:** documentar D como recalibración candidata **medida y validada contra el criterio
sólo para el régimen saturado (n=1607)**, dejando explícito que ese régimen no coincide con ninguna
área operativa. No se toca `solver.py`. La conclusión práctica es que el "desorden" que motivó el
experimento es en buena parte un artefacto de mirar la instancia agregada; sobre los planes reales el
solver ya entrega rutas ordenadas con la config actual. Cualquier adopción de D exigiría primero un
guard de régimen (escalar `T_min` al servicio disponible por ruta) **y** la revisión visual humana de
los GeoJSON A vs D sobre n=1607 —el ojo es el juez final del orden— antes de mover defaults.
