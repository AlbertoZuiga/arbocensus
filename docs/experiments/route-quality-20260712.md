# Comparación de estrategias del solver sobre el dataset real
...
Idéntico a `profiling-20260711.md` §a: `docker-compose.prod.yml`, imágenes
...
1. **Dataset recreado.** El volumen de Postgres del experimento de profiling
   (`profiling-20260711.md`) fue eliminado, así que el dataset se recreó con el mismo code path de
   `POST /api/datasets/from-legacy-selection/` documentado en `profiling-20260711.md` §b
   ...
   `legacy_app`), idéntico en contenido al de `profiling-20260711.md`.
...
- **Fase 1**: 3 estrategias (`global`, `spatial_term`, `cluster_first`) en la
  ...
  (`profiling-20260711.md` §e: 120 s ≈ 180 s en calidad para n=1607); 3 semillas (base 42).
- **Criterio de decisión** (fijado antes de correr): una estrategia gana si
  ...
  El piso 0.85 se calibró con cifras pre-#135 (`profiling-20260711.md`: global 0.865 en esta
...
- Las 3 semillas de cada estrategia convergen a soluciones casi idénticas en k,
  balance y travel (≤0.5 % de spread; consistente con `profiling-20260711.md`: la varianza proviene
...
- El fix de PR #135 (`math.ceil` en el callback) **eliminó el artefacto de excesos de T_max por truncamiento entero del callback**:
  0 rutas >T_max en las 27 corridas post-#135 (en `profiling-20260711.md` eran sistemáticas en
...
- La estructura de la grilla replica la de global en `profiling-20260711.md` §d: T_max=2 h fuerza
  balance ≈1.0 y dispara k (23→36→49 con service_time); T_max=3 h baja k ~30 %
  y el balance a ~0.83.
- Contra la grilla global de `profiling-20260711.md` (180 s, pre-#135, comparación cualitativa),
...
- **Drops en T_max=2 h**: st=2 deja 1 árbol y st=3 deja 2 (los 3 seeds). Es la
  misma zona apretada donde global ya dropeaba con st=3 en `profiling-20260711.md`; con ceil los
...
- k queda estable entre estrategias a igual variante (23/36/49 en T2h,
  idéntico a global en `profiling-20260711.md`): el término espacial no infla la flota.
...
## f. Decisión y defaults operativos recomendados (aplicar en un PR aparte)
...
2. **`SOLVER_TIME_LIMIT_SEC = 120`** (heurística `min(30 + 1.5·n, 120)`):
   confirmado en `profiling-20260711.md` §e (120 s ≈ 180 s en calidad para n=1607) y revalidado
