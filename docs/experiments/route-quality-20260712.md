...
- Branch: `r2-strategy-comparison` (sobre main `55e97d3`, que incluye el fix de
...
- **Fase 1**: 3 estrategias (`global`, `spatial_term`, `cluster_first`) en la
  ...
  (RP2 §e.3: 120 s ≈ 180 s en calidad para n=1607); 3 semillas (base 42).
...
- **spatial_term corta el solapamiento por ruta a menos de la mitad** (168 →
  ...
  (0.833 vs 0.838). El IoU del peor par sube levemente (0.51 → 0.55): las
  cajas se comprimen (sum_rmax −12.8 %) pero persiste un par vecino con alta
  intersección de bbox. Cumple el criterio: reduce solapamiento/ruta, 0 rutas
  >T_max, 0 drops, travel ≪ +10 %.
...
- Las 3 semillas de cada estrategia convergen a soluciones casi idénticas
  (consistente con RP2: la varianza proviene solo del corte por wall-clock).
- El fix de PR #135 (`math.ceil` en el callback) **eliminó el artefacto F7**:
  0 rutas >T_max en las 27 corridas de esta sesión (en RP2 eran sistemáticas
  en T_max=2 h). Como contraparte, la solución de global cambió respecto de la
...
2. **`SOLVER_TIME_LIMIT_SEC = 120`** (heurística `min(30 + 1.5·n, 120)`):
   confirmado en RP2 §e.3 (120 s ≈ 180 s en calidad para n=1607) y revalidado
